...
let LT_MULT2 = prove
 (`!m n p q. m < n /\ p < q ==> m * p < n * q`,
  ...
  UNDISCH_TAC `m < n` THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN SIMP_TAC[LT]);;
...
let num_WOP = prove
 (`!P. (?n. P n) <=> (?n. P(n) /\ !m. m < n ==> ~P(m))`,
  ...
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_EXISTS_THM] THEN
   ...
   [DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC `a:num`) MP_TAC) THEN
    ...
    SPEC_TAC(`m:num`,`m:num`) THEN INDUCT_TAC THENL
     [REWRITE_TAC[LE; LT] THEN DISCH_THEN(IMP_RES_THEN SUBST_ALL_TAC) THEN
      ...
      REWRITE_TAC[LT] THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
   ...
   [CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
    ...
    CONJ_TAC THENL
     [ASM_REWRITE_TAC[MULT_CLAUSES];
      ...
      CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LE] THEN
      REWRITE_TAC[ONE; LT]];
    STRIP_TAC THEN
    FIRST_ASSUM(X_CHOOSE_THEN `d:num` SUBST1_TAC o
...
let LE_EXP = prove
 (`!x m n. x EXP m <= x EXP n <=>
  ...
  CONV_TAC "(EQT_INTRO o TAUT)" (EQT_INTRO o TAUT));;
...
let EQ_EXP = prove
 (`!x m n. x EXP m = x EXP n <=>
  ...
  REWRITE_TAC[GSYM LE_ANTISYM] THEN CONV_TAC "TAUT" TAUT);;
...
let EXP_MONO_EQ = prove
 (`!x y n. x EXP n = y EXP n <=> x = y \/ n = 0`,
  REWRITE_TAC[GSYM LE_ANTISYM; EXP_MONO_LE] THEN CONV_TAC "TAUT" TAUT);;
...
let DIVMOD_EXIST = prove
 (`!m n. ~(n = 0) ==> ?q r. (m = q * n + r) /\ r < n`,
  ...
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN ASM_REWRITE_TAC[NOT_LT] THEN
...
let MOD_MOD = prove
 (`!m n p. ~(n * p = 0) ==> ((m MOD (n * p)) MOD n = m MOD n)`,
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN
...
let MOD_MULT_RMOD = prove
 (`!m n p. ~(n = 0) ==> ((m * (p MOD n)) MOD n = (m * p) MOD n)`,
  REPEAT STRIP_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN
...
let MINIMAL = prove
 (`!P. (?n. P n) <=> P((minimal) P) /\ (!m. m < (minimal) P ==> ~(P m))`,
  GEN_TAC THEN REWRITE_TAC[minimal] THEN CONV_TAC "(RAND_CONV SELECT_CONV)" (RAND_CONV SELECT_CONV) THEN
