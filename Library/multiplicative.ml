...
let MULTIPLICATIVE_CONVOLUTION = prove
 (`!f g. multiplicative f /\ multiplicative g
  ...
  (CONV_TAC "SYM_CONV") SYM_CONV THEN MATCH_MP_TAC NSUM_EQ_GENERAL THEN
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)") (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[IN_ELIM_THM] THEN
 ...
 (`!a b. coprime(a,b) ==> mobius(a * b) = mobius a * mobius b`,
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC FINITE_SUBSET THEN EXISTS_TAC `{p | p divides a * b}` THEN
    ...
    REWRITE_TAC[CONTRAPOS_THM; EXP_2] THEN (CONV_TAC "NUMBER_RULE") NUMBER_RULE]);;
