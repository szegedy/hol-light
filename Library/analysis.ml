...
let REAL_LT_LE = prove(
  ...
  POP_ASSUM MP_TAC THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
...
let REAL_LT_01 = prove(
  ...
  CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
  REWRITE_TAC[REAL_10]);;
...
let REAL_LE_DOUBLE = prove(
  ...
  GEN_TAC THEN EQ_TAC THENL
   [CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LE] THEN
...
let REAL_LINV_UNIQ = prove(
  ...
  DISCH_THEN SUBST1_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
...
let REAL_INV1 = prove(
  `inv(&1) = &1`,
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_LINV_UNIQ THEN
  REWRITE_TAC[REAL_MUL_LID]);;
...
let REAL_LT_NZ = prove(
  ...
  CONV_TAC "(RAND_CONV(ONCE_DEPTH_CONV SYM_CONV))" (RAND_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
...
let REAL_LT_HALF2 = prove(
  ...
  CONV_TAC "(RAND_CONV num_CONV)" (RAND_CONV num_CONV) THEN
...
let REAL_LE_LDIV = prove(
  ...
  SUBGOAL_THEN `y = (y / x) * x` MP_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_RMUL THEN
    CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let REAL_LT_1 = prove(
  ...
  SUBGOAL_THEN `(x / y) < &1 <=> ((x / y) * y) < (&1 * y)` SUBST1_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_LT_RMUL_EQ THEN
    MATCH_MP_TAC REAL_LET_TRANS THEN EXISTS_TAC `x:real` THEN
    ASM_REWRITE_TAC[];
    SUBGOAL_THEN `(x / y) * y = x` SUBST1_TAC THENL
     [MATCH_MP_TAC REAL_DIV_RMUL THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
  ...
  DISCH_THEN(DISJ_CASES_THEN MP_TAC) THENL
   [DISCH_TAC THEN
    ...
    SUBGOAL_THEN `inv(inv x) = x` SUBST1_TAC THENL
     [MATCH_MP_TAC REAL_INVINV THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let REAL_SUMSQ = prove(
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[DE_MORGAN_THM] THEN
...
let ABS_NZ = prove(
  ...
  GEN_TAC THEN EQ_TAC THENL
   [ONCE_REWRITE_TAC[GSYM ABS_ZERO] THEN
    ...
    CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[] THEN
...
let POW_POS_LT = prove(
  ...
  DISCH_TAC THEN CONJ_TAC THENL
   [MATCH_MP_TAC POW_POS THEN ASM_REWRITE_TAC[];
    ...
    CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN ASM_REWRITE_TAC[]]);;
...
let REAL_ARCH = prove(
  ...
  CONV_TAC "(ONCE_DEPTH_CONV NOT_EXISTS_CONV)" (ONCE_DEPTH_CONV NOT_EXISTS_CONV) THEN
...
let SUM_2 = prove
 (`!f n. sum(n,2) f = f(n) + f(n + 1)`,
  REPEAT GEN_TAC THEN CONV_TAC "(REDEPTH_CONV num_CONV)" (REDEPTH_CONV num_CONV) THEN
...
let SUBSETA_ANTISYM = prove(
  ...
  CONV_TAC "(ONCE_DEPTH_CONV AND_FORALL_CONV)" (ONCE_DEPTH_CONV AND_FORALL_CONV) THEN
  REWRITE_TAC[TAUT `(a ==> b) /\ (b ==> a) <=> (a <=> b)`] THEN
  CONV_TAC "(RAND_CONV FUN_EQ_CONV)" (RAND_CONV FUN_EQ_CONV) THEN REFL_TAC);;
...
let CLOSED_LIMPT = prove(
  ...
  REPEAT GEN_TAC THEN CONV_TAC "(ONCE_DEPTH_CONV CONTRAPOS_CONV)" (ONCE_DEPTH_CONV CONTRAPOS_CONV) THEN
  REWRITE_TAC[closed; limpt] THEN
  CONV_TAC "(ONCE_DEPTH_CONV NOT_FORALL_CONV)" (ONCE_DEPTH_CONV NOT_FORALL_CONV) THEN
...
let METRIC_POS = prove(
  ...
  THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LE] THEN
...
let mtop_istopology = prove(
  ...
  CONV_TAC "(REDEPTH_CONV BETA_CONV)" (REDEPTH_CONV BETA_CONV) THEN
...
let ISMET_R1 = prove(
  `ismet (\(x,y). abs(y - x))`,
  REWRITE_TAC[ismet] THEN CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
  CONJ_TAC THEN REPEAT GEN_TAC THENL
   [REWRITE_TAC[ABS_ZERO; REAL_SUB_0] THEN
    CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN REFL_TAC;
...
let MR1_DEF = prove(
  ...
  THEN CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REFL_TAC);;
...
let MTOP_TENDS_UNIQ = prove(
  ...
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_TAC THEN
...
let MR1_BOUNDED = prove(
  ...
  AP_TERM_TAC THEN ABS_TAC THEN
  CONV_TAC "(REDEPTH_CONV EXISTS_AND_CONV)" (REDEPTH_CONV EXISTS_AND_CONV) THEN
...
let NET_NULL_MUL = prove(
  ...
  CONV_TAC "(LAND_CONV LEFT_AND_EXISTS_CONV)" (LAND_CONV LEFT_AND_EXISTS_CONV) THEN
  ...
  SUBGOAL_THEN `e = k * (e / k)` SUBST1_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_DIV_LMUL THEN
    DISCH_THEN SUBST_ALL_TAC THEN UNDISCH_TAC `&0 < &0` THEN
...
let NET_SUB = prove(
  ...
  REWRITE_TAC[real_sub] THEN
  CONV_TAC "(EXACT_CONV[X_BETA_CONV `n:A` `--(y(n:A))`])" (EXACT_CONV[X_BETA_CONV `n:A` `--(y(n:A))`]) THEN
...
let NET_MUL = prove(
  ...
  CONV_TAC "(EXACT_CONV[X_BETA_CONV `n:A` `x(n:A) * (y(n) - y0)`])" (EXACT_CONV[X_BETA_CONV `n:A` `x(n:A) * (y(n) - y0)`]) THEN
  CONV_TAC "(EXACT_CONV[X_BETA_CONV `n:A` `(x(n:A) - x0) * y0`])" (EXACT_CONV[X_BETA_CONV `n:A` `(x(n:A) - x0) * y0`]) THEN
  FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP NET_NULL_ADD) THEN
  GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [REAL_MUL_SYM] THEN
  (CONV_TAC "o" o EXACT_CONV o map (X_BETA_CONV `n:A`))
...
let NET_INV = prove(
  ...
  CONV_TAC "(ONCE_DEPTH_CONV LEFT_AND_EXISTS_CONV)" (ONCE_DEPTH_CONV LEFT_AND_EXISTS_CONV) THEN
  DISCH_THEN(X_CHOOSE_THEN `k:real` MP_TAC) THEN
...
let NET_DIV = prove(
  ...
  CONV_TAC "(EXACT_CONV[X_BETA_CONV `n:A` `inv(y(n:A))`])" (EXACT_CONV[X_BETA_CONV `n:A` `inv(y(n:A))`]) THEN
 ...
 (`!g. dorder g /\
  ...
  FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP NET_ADD) THEN CONJ_TAC THENL
   [FIRST_ASSUM MATCH_MP_TAC THEN
    ...
    CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN FIRST_ASSUM MATCH_MP_TAC THEN
    ARITH_TAC]);;
...
let SEQ_ICONV = prove(
  ...
  ASSUME_TAC THENL
   [FIRST_ASSUM(MP_TAC o SPEC `sup(\x. ?n:num. x = f(n))`) THEN
    ...
    REWRITE_TAC[REAL_NOT_LT] THEN
    CONV_TAC "(ONCE_DEPTH_CONV LEFT_IMP_EXISTS_CONV)" (ONCE_DEPTH_CONV LEFT_IMP_EXISTS_CONV) THEN
    ...
    BETA_TAC THEN DISCH_THEN ASSUME_TAC THEN SUBGOAL_THEN
     ...
     [INDUCT_TAC THENL
       [SUBGOAL_THEN `f(0) >= (SUC N)` MP_TAC THENL
        ...
        CONV_TAC "SELECT_CONV" SELECT_CONV THEN FIRST_ASSUM MATCH_MP_TAC THEN
        FIRST_ASSUM ACCEPT_TAC;
        FIRST_ASSUM(UNDISCH_TAC o
...
let SEQ_INV0 = prove(
  ...
  SUBGOAL_THEN `~(f(n:num) = &0)` ASSUME_TAC THENL
   [CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN MATCH_MP_TAC REAL_LT_IMP_NE THEN
  ...
  SUBGOAL_THEN `e = inv(inv e)` SUBST1_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INVINV THEN
    CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let SEQ_POWER_ABS = prove(
  ...
  REWRITE_TAC[REAL_LE_LT] THEN DISCH_THEN DISJ_CASES_TAC THENL
   [SUBGOAL_THEN `!n. abs(c) pow n = inv(inv(abs(c) pow n))`
      (fun th -> ONCE_REWRITE_TAC[th]) THENL
     [GEN_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INVINV THEN
      MATCH_MP_TAC POW_NZ THEN
      ASM_REWRITE_TAC[ABS_NZ; ABS_ABS]; ALL_TAC] THEN
    CONV_TAC "(EXACT_CONV[X_BETA_CONV `n:num` `inv(abs(c) pow n)`])" (EXACT_CONV[X_BETA_CONV `n:num` `inv(abs(c) pow n)`]) THEN
...
let NEST_LEMMA_UNIQ = prove(
  ...
  CONV_TAC "(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
   ...
   [INDUCT_TAC THENL
    ...
    (SUBGOAL_THEN `~(&2 = &0) /\ ~(&2 pow n = &0)`
      ...
      [CONJ_TAC THENL [ALL_TAC; MATCH_MP_TAC POW_NZ] THEN
       ...
       AP_THM_TAC THEN AP_TERM_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
...
let BOLZANO_LEMMA_ALT = prove
 (`!P. (!a b c. a <= b /\ b <= c /\ P a b /\ P b c ==> P a c) /\
  ...
  REWRITE_TAC[] THEN
  CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[]);;
...
let SUMMABLE_SUM = prove(
  ...
  CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
...
let SER_POS_LT_PAIR = prove(
  ...
  DISCH_THEN(MP_TAC o MATCH_MP SUMMABLE_SUM) THEN
  REWRITE_TAC[sums; SEQ] THEN BETA_TAC THEN
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LT] THEN DISCH_TAC THEN
   ...
   [SPEC_TAC(`N:num`,`N:num`) THEN INDUCT_TAC THENL
    ...
    ASSUME_TAC THENL
     [MATCH_MP_TAC REAL_LE_TRANS THEN
      ...
      CONV_TAC "(REDEPTH_CONV num_CONV)" (REDEPTH_CONV num_CONV) THEN
...
let SER_ADD = prove(
  ...
  CONV_TAC "((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) x`])" ((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) x`]) THEN
  CONV_TAC "((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) y`])" ((RAND_CONV o EXACT_CONV)[X_BETA_CONV `n:num` `sum(0,n) y`]) THEN
...
let GP = prove(
  ...
  REWRITE_TAC[REAL_SUB_0] THEN CONV_TAC "(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
  ASM_REWRITE_TAC[] THEN
...
let ABS_NEG_LEMMA = prove(
  ...
  DISCH_THEN(MP_TAC o MATCH_MP REAL_LE_TRANS) THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
    ...
    EXISTS_TAC `\n. (abs(f N) / c pow N) * (c pow n)` THEN CONJ_TAC THENL
     [EXISTS_TAC `N:num` THEN X_GEN_TAC `n:num` THEN
      ...
      SUBGOAL_THEN `~(c pow N = &0)`
       ...
       [MATCH_MP_TAC POW_NZ THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
        MATCH_MP_TAC REAL_LT_IMP_NE THEN ASM_REWRITE_TAC[]; ALL_TAC] THEN
...
let LIM_CONST = prove(
  ...
  REWRITE_TAC[REAL_SUB_0] THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
  ASM_REWRITE_TAC[]);;
...
let DIFF_CONT = prove(
  ...
  CONV_TAC "(EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`])" (EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`]) THEN
...
let IVT = prove(
  ...
  DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC
   (CONJUNCTS_THEN2 MP_TAC STRIP_ASSUME_TAC)) THEN
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
  DISCH_THEN(ASSUME_TAC o CONV_RULE NOT_EXISTS_CONV) THEN
  ...
  CONJ_TAC THENL
   [MAP_EVERY X_GEN_TAC [`u:real`; `v:real`; `w:real`] THEN
    CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[DE_MORGAN_THM; NOT_IMP] THEN
...
let DIFF_ADD = prove(
  ...
  CONV_TAC "(EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`])" (EXACT_CONV[X_BETA_CONV `h:real` `(f(x + h) - f(x)) / h`]) THEN
  CONV_TAC "(EXACT_CONV[X_BETA_CONV `h:real` `(g(x + h) - g(x)) / h`])" (EXACT_CONV[X_BETA_CONV `h:real` `(g(x + h) - g(x)) / h`]) THEN
...
let DIFF_MUL = prove(
  ...
  CONV_TAC "(EXACT_CONV(map (X_BETA_CONV `h:real`) [`((g(x + h) - g(x)) / h) * f(x + h)`; `((f(x + h) - f(x)) / h) * g(x)`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`((g(x + h) - g(x)) / h) * f(x + h)`; `((f(x + h) - f(x)) / h) * g(x)`])) THEN
  MATCH_MP_TAC LIM_ADD THEN
  CONV_TAC "(EXACT_CONV(map (X_BETA_CONV `h:real`) [`(g(x + h) - g(x)) / h`; `f(x + h):real`; `(f(x + h) - f(x)) / h`; `g(x:real):real`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`(g(x + h) - g(x)) / h`; `f(x + h):real`; `(f(x + h) - f(x)) / h`; `g(x:real):real`])) THEN
...
let DIFF_XM1 = prove(
  ...
  BETA_TAC THEN CONJ_TAC THENL
   [REWRITE_TAC[LIM] THEN X_GEN_TAC `e:real` THEN DISCH_TAC THEN
    ...
    CONV_TAC "(EXACT_CONV[X_BETA_CONV `h:real` `inv(x + h) * inv(x)`])" (EXACT_CONV[X_BETA_CONV `h:real` `inv(x + h) * inv(x)`]) THEN
    REWRITE_TAC[GSYM LIM_NEG] THEN
    CONV_TAC "(EXACT_CONV(map (X_BETA_CONV `h:real`) [`inv(x + h)`; `inv(x)`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`inv(x + h)`; `inv(x)`]))
    THEN MATCH_MP_TAC LIM_MUL THEN BETA_TAC THEN
    REWRITE_TAC[LIM_CONST] THEN
    CONV_TAC "(EXACT_CONV[X_BETA_CONV `h:real` `x + h`])" (EXACT_CONV[X_BETA_CONV `h:real` `x + h`]) THEN
    MATCH_MP_TAC LIM_INV THEN ASM_REWRITE_TAC[] THEN
    GEN_REWRITE_TAC LAND_CONV [GSYM REAL_ADD_RID] THEN
    CONV_TAC "(EXACT_CONV(map (X_BETA_CONV `h:real`) [`x:real`; `h:real`]))" (EXACT_CONV(map (X_BETA_CONV `h:real`) [`x:real`; `h:real`])) THEN
...
let CONT_BOUNDED = prove(
  ...
  X_GEN_TAC `x:real` THEN ASM_CASES_TAC `a <= x /\ x <= b` THENL
   [ALL_TAC;
    ...
    MAP_EVERY X_GEN_TAC [`u:real`; `v:real`] THEN
    REPEAT STRIP_TAC THEN UNDISCH_TAC `~(a <= x /\ x <= b)` THEN
    CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
    REWRITE_TAC[] THEN CONJ_TAC THEN MATCH_MP_TAC REAL_LE_TRANS THENL
     [EXISTS_TAC `u:real`; EXISTS_TAC `v:real`] THEN
    ASM_REWRITE_TAC[]] THEN
  UNDISCH_TAC `!x. a <= x /\ x <= b ==> f contl x` THEN
...
let CONT_ATTAINS = prove(
  ...
  GEN_REWRITE_TAC I [TAUT `a <=> ~ ~a`] THEN
  CONV_TAC "(RAND_CONV NOT_EXISTS_CONV)" (RAND_CONV NOT_EXISTS_CONV) THEN
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN DISCH_TAC THEN
    CONV_TAC "(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN
  ...
  ASM_CASES_TAC `a < x1 /\ x1 < b` THENL
   [FIRST_ASSUM(X_CHOOSE_THEN `d:real` MP_TAC o MATCH_MP INTERVAL_LEMMA) THEN
    ...
    ASM_REWRITE_TAC[] THEN SUBGOAL_THEN
     ...
     [CONV_TAC "EXISTS_AND_CONV" EXISTS_AND_CONV THEN CONJ_TAC THENL
  ...
  SUBGOAL_THEN `!x. a <= x /\ x <= b ==> (f(x):real = f(b))` MP_TAC THENL
   [REPEAT(FIRST_ASSUM(UNDISCH_TAC o check is_neg o concl)) THEN
    ...
    SUBGOAL_THEN `?l. (f diffl l)(x) /\
     ...
     [CONV_TAC "(ONCE_DEPTH_CONV EXISTS_AND_CONV)" (ONCE_DEPTH_CONV EXISTS_AND_CONV) THEN CONJ_TAC THENL
       [REWRITE_TAC[GSYM differentiable] THEN FIRST_ASSUM MATCH_MP_TAC THEN
        ASM_REWRITE_TAC[];
        EXISTS_TAC `d:real` THEN ASM_REWRITE_TAC[] THEN GEN_TAC THEN
        DISCH_THEN(fun th -> FIRST_ASSUM(MP_TAC o C MATCH_MP th)) THEN
...
let MVT = prove(
  ...
  funpow 2 (fst o dest_imp) o snd) THENL
   [ASM_REWRITE_TAC[MVT_LEMMA] THEN BETA_TAC THEN
    CONJ_TAC THEN X_GEN_TAC `x:real` THENL
     [DISCH_TAC THEN CONV_TAC "(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN
      MATCH_MP_TAC CONT_SUB THEN CONJ_TAC THENL
       [CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
        FIRST_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[];
        CONV_TAC "(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN MATCH_MP_TAC CONT_MUL THEN
      ...
      CONV_TAC "(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN MATCH_MP_TAC DIFF_SUB THEN
      CONJ_TAC THENL
       [CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN FIRST_ASSUM ACCEPT_TAC;
        CONV_TAC "(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN REWRITE_TAC[] THEN
  ...
  ASM_REWRITE_TAC[] THEN DISCH_THEN((then_) CONJ_TAC o MP_TAC) THENL
   [ALL_TAC; DISCH_THEN(K ALL_TAC) THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
  ...
  REWRITE_TAC[REAL_SUB_ADD] THEN CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
  REWRITE_TAC[REAL_ADD_LID]);;
...
let DIFF_ISCONST_ALL = prove(
  ...
  REPEAT GEN_TAC THEN REPEAT_TCL DISJ_CASES_THEN MP_TAC
   ...
   [DISCH_THEN SUBST1_TAC THEN REFL_TAC;
    CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV);
    ALL_TAC] THEN
     ...
     [X_GEN_TAC `z:real` THEN STRIP_TAC THEN FIRST_ASSUM MATCH_MP_TAC THEN
      ...
      SUBGOAL_THEN `g(f(x - d):real) = x - d` SUBST1_TAC THENL
       [FIRST_ASSUM MATCH_MP_TAC THEN
        ...
        ASM_REWRITE_TAC[real_abs; REAL_LE_REFL]; ALL_TAC] THEN
      REWRITE_TAC[] THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
      MATCH_MP_TAC REAL_LT_IMP_NE THEN
      MATCH_MP_TAC REAL_LTE_TRANS THEN EXISTS_TAC `x:real` THEN
...
let DIFF_INVERSE = prove(
   ...
   [ALL_TAC;
    ...
    SUBGOAL_THEN `y - f(x) = h(g(y)) * (g(y) - x)` SUBST1_TAC THENL
     [FIRST_ASSUM(fun th -> GEN_REWRITE_TAC RAND_CONV [GSYM th]) THEN
      ...
      MATCH_MP_TAC EQ_IMP THEN
      AP_THM_TAC THEN AP_TERM_TAC THEN
      CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[ABS_ZERO; REAL_SUB_0]] THEN
  ...
  DISCH_THEN(fun th -> FIRST_ASSUM(MP_TAC o C MATCH_MP th)) THEN
  REWRITE_TAC[GSYM ABS_NZ] THEN
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN ASM_REWRITE_TAC[] THEN
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC[REAL_SUB_LZERO; ABS_NEG; REAL_LT_REFL]);;
