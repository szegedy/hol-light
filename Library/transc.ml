...
let POWDIFF = prove(
  ...
  INDUCT_TAC THENL
   [REPEAT GEN_TAC THEN REWRITE_TAC[sum] THEN
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[REAL_ADD_LID_UNIQ] THEN
...
let POWSER_INSIDEA = prove(
  ...
  EXISTS_TAC `\n. (K * abs(z pow n)) / abs(x pow n)` THEN CONJ_TAC THENL
   [EXISTS_TAC `0` THEN X_GEN_TAC `n:num` THEN DISCH_THEN(K ALL_TAC) THEN
    ...
    MATCH_MP_TAC POW_INV THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let TERMDIFF_LEMMA1 = prove(
  ...
  REWRITE_TAC[REAL_SUB_LDISTRIB; GSYM POW_ADD] THEN BINOP_TAC THENL
   [MATCH_ACCEPT_TAC REAL_MUL_SYM;
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUB_ADD THEN
...
let TERMDIFF_LEMMA2 = prove(
  ...
  AP_TERM_TAC THEN AP_TERM_TAC THEN CONV_TAC "(TOP_DEPTH_CONV num_CONV)" (TOP_DEPTH_CONV num_CONV) THEN
...
let TERMDIFF = prove(
  ...
  DISCH_THEN MATCH_MP_TAC THEN REPEAT CONJ_TAC THENL
   [ASM_REWRITE_TAC[REAL_SUB_LT];
    ...
    CONV_TAC "(X_FUN_EQ_CONV `n:num`)" (X_FUN_EQ_CONV `n:num`) THEN BETA_TAC THEN GEN_TAC THEN
    REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
    ...
    MATCH_MP_TAC REAL_MUL_RINV THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
...
let POWSER_0 = prove
 (`!a. (\n. a n * (&0) pow n) sums a(0)`,
  ...
  SUBGOAL_THEN `a(0) = sum(0,1) (\n. a n * (&0) pow n)` SUBST1_TAC THENL
   [CONV_TAC "(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
...
let POWSER_LIMIT_0 = prove
 (`!f a s. &0 < s /\
  ...
  SUBGOAL_THEN `suminf (\n. a n * &0 pow n) = a(0)` SUBST1_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNIQ THEN
...
let POW_EQ = prove(
  ...
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THENL
   [ALL_TAC; CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV)] THEN
  MATCH_MP_TAC REAL_LT_IMP_NE THEN
...
let REAL_EXP_FDIFF = prove(
  ...
  CONV_TAC "(X_FUN_EQ_CONV `n:num`)" (X_FUN_EQ_CONV `n:num`) THEN GEN_TAC THEN BETA_TAC THEN
  REWRITE_TAC[FACT; GSYM REAL_MUL] THEN
  SUBGOAL_THEN `~(&(SUC n) = &0) /\ ~(&(FACT n) = &0)` ASSUME_TAC THENL
   [CONJ_TAC THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
    MATCH_MP_TAC REAL_LT_IMP_NE THEN
...
let DIFF_EXP = prove(
  ...
  CONV_TAC "(LAND_CONV BETA_CONV)" (LAND_CONV BETA_CONV) THEN
...
let REAL_EXP_0 = prove(
  `exp(&0) = &1`,
  REWRITE_TAC[exp] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
   ...
   [MP_TAC(SPECL [`\n. ((\n. inv(&(FACT n)))) n * (x pow n)`; `2`]
    ...
    funpow 2 (fst o dest_imp) o snd) THENL
     [GEN_TAC THEN DISCH_THEN(K ALL_TAC) THEN
      ...
      CONV_TAC "(TOP_DEPTH_CONV num_CONV)" (TOP_DEPTH_CONV num_CONV) THEN REWRITE_TAC[sum] THEN
...
let REAL_EXP_ADD_MUL = prove(
  ...
  REPEAT GEN_TAC THEN
  CONV_TAC "(LAND_CONV(X_BETA_CONV `x:real`))" (LAND_CONV(X_BETA_CONV `x:real`)) THEN
...
let REAL_EXP_MONO_LT = prove(
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[REAL_NOT_LT] THEN
     ...
     [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNIQ THEN
      ...
      SUBGOAL_THEN `(&1 - x) * inv (&1 - x) = &1` SUBST1_TAC THENL
       [MATCH_MP_TAC REAL_MUL_RINV THEN
        ...
        POP_ASSUM MP_TAC THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
        CONJ_TAC THENL
...
let LN_EXP = prove(
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN CONV_TAC "(RAND_CONV(ONCE_DEPTH_CONV SYM_CONV))" (RAND_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
  CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN MATCH_MP_TAC SELECT_AX THEN
...
let LN_INJ = prove(
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_ACCEPT_TAC REAL_EXP_INJ);;
...
let LN_MONO_LT = prove(
  ...
  EVERY_ASSUM(fun th -> GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
    [SYM(REWRITE_RULE[GSYM REAL_EXP_LN] th)]) THEN
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_ACCEPT_TAC REAL_EXP_MONO_LT);;
...
let LN_MONO_LE = prove(
  ...
  EVERY_ASSUM(fun th -> GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
    [SYM(REWRITE_RULE[GSYM REAL_EXP_LN] th)]) THEN
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_ACCEPT_TAC REAL_EXP_MONO_LE);;
     ...
     [REWRITE_TAC[real_div; GSYM REAL_MUL_ASSOC] THEN
      SUBGOAL_THEN `inv(&(SUC n)) * &(SUC n) = &1` SUBST1_TAC THENL
       [MATCH_MP_TAC REAL_MUL_LINV THEN REWRITE_TAC[REAL_INJ; NOT_SUC];
        ...
        CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC LN_POW THEN
        ASM_REWRITE_TAC[]];
...
let POW_ROOT_POS = prove(
  ...
  DISCH_TAC THEN ASM_REWRITE_TAC[] THENL
   [DISJ_CASES_THEN MP_TAC (REWRITE_RULE[REAL_LE_LT] (ASSUME `&0 <= x`)) THENL
    ...
    ASM_REWRITE_TAC[REAL_LT_LE] THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let ROOT_MONO_LT = prove
 (`!x y. &0 <= x /\ x < y ==> root(SUC n) x < root(SUC n) y`,
  ...
  UNDISCH_TAC `x < y` THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let REAL_DIV_SQRT = prove
 (`!x. &0 <= x ==> (x / sqrt(x) = sqrt(x))`,
  ...
  DISCH_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SQRT_POS_UNIQ THEN
 ...
 (`!x. &0 < x ==> (sqrt diffl inv(&2 * sqrt(x))) x`,
  ...
  REWRITE_TAC[REAL_OF_NUM_EQ; ARITH_EQ] THEN REPEAT CONJ_TAC THENL
   [ASM_MESON_TAC[POW_2_SQRT; REAL_ARITH `abs(x - y) < y ==> &0 <= x`];
    REPEAT STRIP_TAC THEN CONV_TAC "CONTINUOUS_CONV" CONTINUOUS_CONV;
...
let SIN_0 = prove(
  `sin(&0) = &0`,
  REWRITE_TAC[sin] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
  MATCH_MP_TAC SUM_UNIQ THEN BETA_TAC THEN
  W(MP_TAC o C SPEC SER_0 o rand o rator o snd) THEN
...
let COS_0 = prove(
  `cos(&0) = &1`,
  REWRITE_TAC[cos] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
  MATCH_MP_TAC SUM_UNIQ THEN BETA_TAC THEN
  W(MP_TAC o C SPEC SER_0 o rand o rator o snd) THEN
  DISCH_THEN(MP_TAC o SPEC `1`) THEN
...
let SIN_CIRCLE = prove(
  ...
  GEN_TAC THEN CONV_TAC "(LAND_CONV(X_BETA_CONV `x:real`))" (LAND_CONV(X_BETA_CONV `x:real`)) THEN
...
let SIN_POS = prove(
  ...
  ASM_REWRITE_TAC[] THEN CONV_TAC "(REDEPTH_CONV num_CONV)" (REDEPTH_CONV num_CONV) THEN
...
let SIN_PI2 = prove(
  ...
  POP_ASSUM MP_TAC THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_THEN(K ALL_TAC) THEN
...
let SIN_PI = prove(
  ...
  REWRITE_TAC[REAL_DOUBLE] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
...
let COS_TOTAL = prove(
  ...
  GEN_TAC THEN STRIP_TAC THEN
  CONV_TAC "EXISTS_UNIQUE_CONV" EXISTS_UNIQUE_CONV THEN CONJ_TAC THENL
...
let SIN_TOTAL = prove(
  ...
  CONV_TAC "(ONCE_DEPTH_CONV EXISTS_UNIQUE_CONV)" (ONCE_DEPTH_CONV EXISTS_UNIQUE_CONV) THEN
...
let COS_ONE_2PI = prove
 (`!x. (cos(x) = &1) <=> (?n. x = &n * &2 * pi) \/ (?n. x = --(&n * &2 * pi))`,
  ...
  COND_CASES_TAC THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[]);;
...
let DIFF_TAN = prove(
  ...
  CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN ONCE_REWRITE_TAC[REAL_ADD_SYM] THEN
...
let TAN_TOTAL_LEMMA = prove(
  ...
  MP_TAC THENL
   [SUBST1_TAC(SYM(SPEC `&1` REAL_DIV_LZERO)) THEN
    CONV_TAC "(ONCE_DEPTH_CONV HABS_CONV)" (ONCE_DEPTH_CONV HABS_CONV) THEN MATCH_MP_TAC LIM_DIV THEN
    REWRITE_TAC[REAL_10] THEN CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)" (ONCE_DEPTH_CONV ETA_CONV) THEN
...
let TAN_TOTAL = prove(
  ...
  GEN_TAC THEN CONV_TAC "EXISTS_UNIQUE_CONV" EXISTS_UNIQUE_CONV THEN CONJ_TAC THENL
   [DISJ_CASES_TAC(SPEC `y:real` REAL_LE_NEGTOTAL) THEN
    ...
    REPEAT_TCL DISJ_CASES_THEN ASSUME_TAC
     ...
     [DISCH_THEN(K ALL_TAC) THEN POP_ASSUM ACCEPT_TAC;
      ...
      CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN ONCE_REWRITE_TAC[CONJ_SYM]] THEN
    (STRIP_TAC THEN MP_TAC(SPECL [`tan`; `x1:real`; `x2:real`] ROLLE) THEN
     ASM_REWRITE_TAC[] THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let TAN_PI4 = prove
 (`tan(pi / &4) = &1`,
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN MATCH_MP_TAC REAL_MUL_RINV THEN
...
let COS_ATN_NZ = prove(
  ...
  CONV_TAC "NOT_EXISTS_CONV" NOT_EXISTS_CONV THEN X_GEN_TAC `n:num` THEN
...
let COS_SIN_SQRT = prove(
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC POW_ROOT_POS THEN
  ASM_REWRITE_TAC[]);;
...
let COS_ASN_NZ = prove(
  ...
  CONJ_TAC THEN CONV_TAC "NOT_EXISTS_CONV" NOT_EXISTS_CONV THEN
  ...
  REWRITE_TAC[NOT_SUC] THEN EQ_TAC THENL
   [DISCH_TAC THEN MATCH_MP_TAC LESS_EQUAL_ANTISYM THEN CONJ_TAC THENL
     [POP_ASSUM(MP_TAC o SPEC `1` o CONJUNCT1) THEN
      ...
      REWRITE_TAC[LE_0] THEN CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN
   ...
   [MATCH_MP_TAC REAL_LE_TRANS THEN EXISTS_TAC `(D:num->real) r` THEN
    ...
    MATCH_MP_TAC REAL_LE_TRANS THEN EXISTS_TAC `b:real` THEN CONJ_TAC THENL
     [MATCH_MP_TAC DIVISION_LE THEN
      ...
      MATCH_MP_TAC REAL_EQ_IMP_LE THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
   ...
   [RULE_ASSUM_TAC(REWRITE_RULE[tdiv]) THEN
    ...
    SUBGOAL_THEN `c = (\n. if (n < (dsize D1)) then  D1(n) else D2(n -
     ...
     [BETA_TAC THEN REWRITE_TAC[GSYM NOT_LE; LE_ADD] THEN
      ONCE_REWRITE_TAC[ADD_SYM] THEN REWRITE_TAC[ADD_SUB] THEN
      CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC DIVISION_RHS THEN
...
let DINT_UNIQ = prove(
  ...
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN ONCE_REWRITE_TAC[ABS_NZ] THEN DISCH_TAC THEN
...
let INTEGRABLE_DEFINT = prove
 (`!f a b. integrable(a,b) f ==> defint(a,b) f (integral(a,b) f)`,
  ...
  CONV_TAC "(RAND_CONV SELECT_CONV)" (RAND_CONV SELECT_CONV) THEN REWRITE_TAC[]);;
...
let MCLAURIN = prove(
  ...
  SUBGOAL_THEN `?B. f(h) = sum(0,n)(\m. (diff(m)(&0) / &(FACT m)) * (h pow m))
   ...
   [ONCE_REWRITE_TAC[REAL_ADD_SYM] THEN
    ...
    AP_TERM_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
    ONCE_REWRITE_TAC[AC REAL_MUL_AC
  ...
  SUBGOAL_THEN `difg(0):real->real = g` ASSUME_TAC THENL
   [EXPAND_TAC "difg" THEN BETA_TAC THEN EXPAND_TAC "g" THEN
    CONV_TAC "FUN_EQ_CONV" FUN_EQ_CONV THEN GEN_TAC THEN BETA_TAC THEN
  ...
  SUBGOAL_THEN `(!m t. m < n /\ (& 0) <= t /\ t <= h ==>
   ...
   [REPEAT GEN_TAC THEN DISCH_TAC THEN EXPAND_TAC "difg" THEN BETA_TAC THEN
    CONV_TAC "((funpow 2 RATOR_CONV o RAND_CONV) HABS_CONV)" ((funpow 2 RATOR_CONV o RAND_CONV) HABS_CONV) THEN
    ...
    MATCH_MP_TAC DIFF_ADD THEN CONJ_TAC THENL
     [ALL_TAC;
      ...
      MATCH_MP_TAC EQ_IMP THEN
      AP_THM_TAC THEN CONV_TAC "(ONCE_DEPTH_CONV(ALPHA_CONV `t:real`))" (ONCE_DEPTH_CONV(ALPHA_CONV `t:real`)) THEN
    ...
    CONV_TAC "(funpow 2 RATOR_CONV (RAND_CONV HABS_CONV))" (funpow 2 RATOR_CONV (RAND_CONV HABS_CONV)) THEN
    ...
    DISCH_THEN(MP_TAC o SPEC `t:real`) THEN
    MATCH_MP_TAC EQ_IMP THEN
    CONV_TAC "(ONCE_DEPTH_CONV(ALPHA_CONV `z:real`))" (ONCE_DEPTH_CONV(ALPHA_CONV `z:real`)) THEN
    AP_THM_TAC THEN AP_TERM_TAC THEN
    ...
    IMP_SUBST_TAC(GSYM REAL_INV_MUL_WEAK) THENL
     [CONV_TAC "(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN REWRITE_TAC[REAL_FACT_NZ] THEN
 ...
 (`!f diff h n.
  ...
  funpow 2 (fst o dest_imp) o snd) THENL
   [REPEAT GEN_TAC THEN
    ...
    CONV_TAC "(ONCE_DEPTH_CONV(ALPHA_CONV `z:real`))" (ONCE_DEPTH_CONV(ALPHA_CONV `z:real`)) THEN
    AP_THM_TAC THEN AP_TERM_TAC THEN
    CONV_TAC "(AC REAL_MUL_AC)" (AC REAL_MUL_AC);
    DISCH_THEN(X_CHOOSE_THEN `t:real` STRIP_ASSUME_TAC)] THEN
 ...
 (`!x n. abs(sin x -
  ...
  W(C SUBGOAL_THEN (fun th -> REWRITE_TAC[th]) o funpow 2 lhand o snd) THENL
   [CONJ_TAC THENL
    ...
    SPEC_TAC(`d:num`,`d:num`) THEN CONV_TAC "EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN
    CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `r:num` THEN STRIP_TAC THEN
    ...
    SPEC_TAC(`r MOD 4`,`d:num`) THEN CONV_TAC "EXPAND_CASES_CONV" EXPAND_CASES_CONV THEN
    CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[] THEN
...
let REAL_ATN_POWSER_DIFFS_SUM = prove
 (`!x. abs(x) < &1
  ...
  SUBST1_TAC THENL
   [ABS_TAC THEN
    CONV_TAC "(LAND_CONV(LAND_CONV(RAND_CONV(TOP_DEPTH_CONV num_CONV))))" (LAND_CONV(LAND_CONV(RAND_CONV(TOP_DEPTH_CONV num_CONV)))) THEN
 ...
 (`!x n. abs(x) < &1
  ...
  ASSUME_TAC THENL
   [GEN_TAC THEN COND_CASES_TAC THEN
    ...
    MATCH_MP_TAC REAL_LE_INV2 THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
