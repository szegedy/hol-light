...
let PERMUTES_INSERT = prove
 (`{p:A->A | p permutes (a INSERT s)} =
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)") (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN EQ_TAC THENL
...
let HAS_SIZE_PERMUTATIONS = prove
 (`!s:A->bool n. s HAS_SIZE n ==> {p | p permutes s} HAS_SIZE (FACT n)`,
  ...
  REWRITE_TAC[NOT_IN_EMPTY] THEN CONJ_TAC THENL
   [GEN_TAC THEN DISCH_THEN(SUBST1_TAC o SYM) THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV;
    ALL_TAC] THEN
