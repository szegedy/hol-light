...
let DIVIDES_ONE = prove(
  ...
  CONV_TAC "(LAND_CONV(ONCE_DEPTH_CONV SYM_CONV))" (LAND_CONV(ONCE_DEPTH_CONV SYM_CONV)) THEN
...
let IND_EUCLID = prove(
  ...
  DISCH_THEN(fun th -> SUBST1_TAC(SYM(MATCH_MP SUB_ADD
   ...
   [DISCH_THEN SUBST1_TAC THEN
    FIRST_ASSUM (CONV_TAC "o" o REWR_CONV) THEN
...
let COPRIME = prove(
  ...
  REPEAT(EQ_TAC ORELSE STRIP_TAC) THEN ASM_REWRITE_TAC[DIVIDES_1] THENL
   [FIRST_ASSUM MATCH_MP_TAC;
    FIRST_ASSUM(CONV_TAC "o" o REWR_CONV o GSYM) THEN CONJ_TAC] THEN
  ASM_REWRITE_TAC[]);;
...
let COPRIME_REXP = prove
 (`!m n k. coprime(m,n EXP k) <=> coprime(m,n) \/ k = 0`,
  ...
  REWRITE_TAC[EXP] THEN CONV_TAC "NUMBER_RULE" NUMBER_RULE);;
...
let PRIME_GE_2 = prove(
  `!p. prime(p) ==> 2 <= p`,
  GEN_TAC THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LE] THEN
  REWRITE_TAC[num_CONV `2`; num_CONV `1`; LESS_THM; NOT_LESS_0] THEN
...
let COPRIME_PRIME_EQ = prove(
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [DISCH_THEN(fun th -> REWRITE_TAC[MATCH_MP COPRIME_PRIME th]);
    CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[coprime] THEN
...
let DIVIDES_EXP_PLUS1 = prove
 (`!n k. ODD k ==> (n + 1) divides (n EXP k + 1)`,
  ...
  INDUCT_TAC THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
 ...
 (`!k n. (n - 1) divides (n EXP k - 1)`,
  REPEAT GEN_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [STRUCT_CASES_TAC(SPEC `k:num` num_CASES) THEN
    ASM_REWRITE_TAC[EXP; MULT_CLAUSES] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REWRITE_TAC[DIVIDES_REFL];
...
let PRIME_IRREDUCIBLE = prove
 (`!p. prime p <=>
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "NUM_RING" NUM_RING);;
   ...
   [ASM_REWRITE_TAC[EXP_ZERO; MULT_EQ_0] THEN
    ...
    (ANTS_TAC THENL
      [MATCH_MP_TAC(NUM_RING `!p. ~(p = 0) /\ a * p = b * p ==> a = b`) THEN
       ...
       FIRST_X_ASSUM(MP_TAC o SYM) THEN CONV_TAC "NUM_RING" NUM_RING;
...
let INDEX_FACT_UNBOUNDED = prove
 (`!p n. prime p
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC NSUM_SUPERSET THEN
...
let LCM = prove
 (`!m n. m divides lcm(m,n) /\
  ...
  ASM_SIMP_TAC[GCD_ZERO; LE_1; ADD_CLAUSES] THEN CONV_TAC "SYM_CONV" SYM_CONV THENL
...
let DIVIDES_LCM_GCD = prove
 (`!m n d. d divides lcm(m,n) <=> d * gcd(m,n) divides m * n`,
  ...
  ASM_SIMP_TAC[DIV_MULT] THEN CONV_TAC "NUMBER_RULE" NUMBER_RULE);;
...
let LCM_ASSOC = prove
 (`!m n p. lcm(m,lcm(n,p)) = lcm(lcm(m,n),p)`,
  ...
  REWRITE_TAC[LCM_ZERO] THEN CONV_TAC "TAUT" TAUT);;
...
let LCM_GCD_DISTRIB = prove
 (`!a b c. lcm(a,gcd(b,c)) = gcd(lcm(a,b),lcm(a,c))`,
  ...
  SIMP_TAC[PRIMEPOW_DIVIDES_LCM; DIVIDES_GCD] THEN CONV_TAC "TAUT" TAUT);;
...
let LCM_LMUL = prove
 (`!a b c. lcm(c * a,c * b) = c * lcm(a,b)`,
  ...
  CONV_TAC "NUMBER_RULE" NUMBER_RULE);;
...
let LCM_EXP = prove
 (`!n a b. lcm(a EXP n,b EXP n) = lcm(a,b) EXP n`,
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM DIVIDES_DIV_MULT] THEN
  CONV_TAC "NUMBER_RULE" NUMBER_RULE);;
...
let GCD_CONV =
  ...
  let pth1 = prove
   (`!m n x y d m' n'.
    ...
    CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN ASM_REWRITE_TAC[GSYM GCD_UNIQUE] THEN
