     ...
     [MATCH_MP_TAC REAL_LT_LMUL THEN
      ...
      MATCH_MP_TAC REAL_LT_LMUL THEN CONJ_TAC THENL
       [REWRITE_TAC[GSYM REAL_ABS_NZ; REAL_POW_EQ_0] THEN
        CONV_TAC "(RAND_CONV(LAND_CONV REAL_INT_EQ_CONV))" (RAND_CONV(LAND_CONV REAL_INT_EQ_CONV)) THEN REWRITE_TAC[];
         ...
         [MATCH_MP_TAC REAL_LE_LCANCEL_IMP THEN
          ...
          EXISTS_TAC `abs(a)` THEN CONJ_TAC THENL
           [UNDISCH_TAC `?m. (a = &m) \/ (a = -- (&m))` THEN
            ...
            CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LT; LE];
...
let REALCALC_ADD = prove
 (`(n + 2 = n') ==>
  ...
  CONV_TAC "(LAND_CONV REAL_INT_REDUCE_CONV)" (LAND_CONV REAL_INT_REDUCE_CONV) THEN
...
let REALCALC_MUL = prove
 (`abs(a - &2 pow k * x) < &1 ==>
  ...
  CONV_TAC "(LAND_CONV (EQT_INTRO o REAL_ARITH))" (LAND_CONV (EQT_INTRO o REAL_ARITH)) THEN
  REWRITE_TAC[REAL_MUL_RID] THEN
  GEN_REWRITE_TAC RAND_CONV [REAL_MUL_2] THEN
  MATCH_MP_TAC REAL_LET_TRANS THEN
  EXISTS_TAC `&2 * abs(&2 pow m * c - a * b) +
              &2 * abs(a * b - &2 pow m * &2 pow n * x * y)` THEN
  CONV_TAC "(LAND_CONV (EQT_INTRO o REAL_ARITH))" (LAND_CONV (EQT_INTRO o REAL_ARITH)) THEN REWRITE_TAC[] THEN
...
let REALCALC_SQRT = prove
 (`abs(a - &2 pow n * x) < &1
  ...
  UNDISCH_TAC `&1 <= x` THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
  REWRITE_TAC[REAL_NOT_LE] THEN DISCH_TAC THEN
  SUBGOAL_THEN `x = sqrt(x) pow 2` SUBST1_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN ASM_REWRITE_TAC[SQRT_POW2];
   ...
   [AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
    ...
    MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_MUL_ASSOC] THEN MATCH_MP_TAC REAL_LE_RMUL THEN
      CONV_TAC "(RAND_CONV (EQT_INTRO o REAL_ARITH))" (RAND_CONV (EQT_INTRO o REAL_ARITH)) THEN REWRITE_TAC[] THEN
      GEN_REWRITE_TAC RAND_CONV [GSYM REAL_MUL_LID] THEN
      MATCH_MP_TAC REAL_LE_MUL2 THEN REPEAT CONJ_TAC THENL
       ...
       [MATCH_MP_TAC REAL_MUL_LINV THEN REWRITE_TAC[REAL_OF_NUM_EQ; NOT_SUC];
        ...
        CONJ_TAC THENL
         [MATCH_MP_TAC REAL_LE_INV2 THEN REWRITE_TAC[REAL_POS] THEN
          ...
          REPEAT CONJ_TAC THENL
           [CONV_TAC "(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN ASM_REWRITE_TAC[];
   ...
   [REWRITE_TAC[ADD_CLAUSES] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC(GEN_ALL STEP_EXP) THEN ASM_REWRITE_TAC[] THEN
      ...
      CONV_TAC "(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN REWRITE_TAC[] THEN
      MATCH_MP_TAC REAL_LE_ADD2 THEN CONJ_TAC THENL
       [ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
...
let STEPS_EXP_1 = prove
 (`abs(x) <= &1 /\
   ...
   ==> abs(sum(0,1) t - &2 pow n * sum(0,1)(\i. x pow i / &(FACT i))) 
         <= &2 * &1`,
  CONV_TAC "(ONCE_DEPTH_CONV EXPAND_RANGE_CONV)" (ONCE_DEPTH_CONV EXPAND_RANGE_CONV) THEN REWRITE_TAC[] THEN
  STRIP_TAC THEN CONV_TAC "(ONCE_DEPTH_CONV REAL_SUM_CONV)" (ONCE_DEPTH_CONV REAL_SUM_CONV) THEN
 ...
 (`2 <= n /\
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_LE_TRANS THEN EXISTS_TAC `inv(&2) pow (SUC p)` THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_POW_LE2 THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
      ASM_REWRITE_TAC[REAL_ABS_POS];
...
let REAL_EXP_15 = prove
 (`exp(&1) < &5`,
  ...
  CONV_TAC "(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN
  REWRITE_TAC[] THEN
  MATCH_MP_TAC REAL_LE_MUL2 THEN
...
let REAL_EXP_13 = prove
 (`exp(&1) < &3`,
  ...
  REWRITE_TAC[ADD_CLAUSES] THEN
  CONV_TAC "(ONCE_DEPTH_CONV NUM_FACT_CONV)" (ONCE_DEPTH_CONV NUM_FACT_CONV) THEN
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  MATCH_MP_TAC(REAL_ARITH
...
let APPROX_LEMMA1 = prove
 (`abs(f(x:real) - sum(0,m) (\i. P i x)) < inv(&2 pow (n + 2)) /\
  ...
  CONV_TAC "(funpow 4 RAND_CONV num_CONV)" (funpow 4 RAND_CONV num_CONV) THEN
