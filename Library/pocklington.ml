 ...
 (`!x y n. ~(n = 0) ==> ((x == y) (mod n) <=> (x MOD n = y MOD n))`,
  ...
  REPEAT STRIP_TAC THEN EQ_TAC THEN STRIP_TAC THENL
   [ASM_CASES_TAC `x <= y` THENL
     [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MOD_EQ THEN EXISTS_TAC `q1 - q2`;
 ...
 (`!n. (phi n = n - 1) /\ ~(n = 0) /\ ~(n = 1) <=> prime n`,
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    REWRITE_TAC[COPRIME_SYM] THEN CONV_TAC "TAUT" TAUT] THEN
...
let PHI_2 = prove
 (`phi 2 = 1`,
  SIMP_TAC[PHI_PRIME; PRIME_2] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV);;
...
let PHI_MULTIPLICATIVE = prove
 (`!a b. coprime(a,b) ==> phi(a * b) = phi(a) * phi(b)`,
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_IMAGE_INJ_EQ THEN
 ...
 (`!n. EVEN(phi n) <=> n = 0 \/ 3 <= n`,
  GEN_TAC THEN EQ_TAC THENL
   [ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN
    ...
    STRIP_TAC THEN ASM_REWRITE_TAC[PHI_1; PHI_2] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV;
...
let ITERATE_OVER_COPRIME = prove
 (`!op f n k.
  ...
  UNDISCH_TAC `(k * m == 1) (mod n)` THEN CONV_TAC "NUMBER_RULE" NUMBER_RULE);;
...
let FERMAT_LITTLE = prove
 (`!a n. coprime(a,n) ==> (a EXP (phi n) == 1) (mod n)`,
  ...
  ASM_SIMP_TAC[COPRIME_0; PHI_0; CONG_MOD_0] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
...
let LUCAS = prove
 (`!a n. 2 <= n /\
  ...
  SUBGOAL_THEN `~(r = 1)` MP_TAC THENL
   [UNDISCH_TAC `m < m * r` THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
...
let ORDER_DIVIDES = prove
 (`!n a d. (a EXP d == 1) (mod n) <=> order(n) a divides d`,
  GEN_TAC THEN GEN_TAC THEN REWRITE_TAC[order] THEN CONV_TAC "SELECT_CONV" SELECT_CONV THEN
...
let ORDER_UNIQUE = prove
 (`!n a k. 0 < k /\
  ...
  UNDISCH_TAC `(a EXP k == 1) (mod n)` THEN CONV_TAC "NUMBER_RULE" NUMBER_RULE);;
