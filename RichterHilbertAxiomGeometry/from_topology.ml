...
let SPHERE_EQ_EMPTY = prove
 (`!a:real^N r. sphere(a,r) = {} <=> r < &0`,
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL [ALL_TAC; CONV_TAC "NORM_ARITH" NORM_ARITH] THEN
...
let LIMPT_OF_UNIV = prove
 (`!x. x limit_point_of (:real^N)`,
  ...
  POP_ASSUM MP_TAC THEN CONV_TAC "NORM_ARITH" NORM_ARITH);;
 ...
 (`!s x:real^N. x IN interior s ==> x limit_point_of s`,
  ...
  REPEAT CONJ_TAC THENL
   [FIRST_X_ASSUM MATCH_MP_TAC;
    CONV_TAC "(RAND_CONV SYM_CONV)" (RAND_CONV SYM_CONV) THEN REWRITE_TAC[GSYM DIST_EQ_0];
...
let LIMPT_SEQUENTIAL_INJ = prove
 (`!x:real^N s.
  ...
  ASSUME_TAC THENL
   [INDUCT_TAC THEN ASM_REWRITE_TAC[] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let SPHERE_EQ_SING = prove
 (`!a:real^N r x. sphere(a,r) = {x} <=> x = a /\ r = &0`,
  ...
  REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "NORM_ARITH" NORM_ARITH);;
...
let INTER_BALLS_EQ_EMPTY = prove
 (`(!a b:real^N r s. ball(a,r) INTER ball(b,s) = {} <=>
  ...
  (EQ_TAC THENL
    [ALL_TAC;
     SPEC_TAC(`b % basis 1:real^N`,`v:real^N`) THEN CONV_TAC "NORM_ARITH" NORM_ARITH]) THEN
...
let CONTINUOUS_ON_COMPONENTS_GEN = prove
 (`!f:real^M->real^N s.
  ...
  SUBST1_TAC THENL
   [CONV_TAC "(LAND_CONV(SUBS_CONV [ISPEC `s:real^M->bool` UNIONS_COMPONENTS]))" (LAND_CONV(SUBS_CONV [ISPEC `s:real^M->bool` UNIONS_COMPONENTS])) THEN
 ...
 (`!s:real^N->bool k.
  ...
  EQ_TAC THENL
   [DISCH_THEN(X_CHOOSE_THEN `x:real^N`
    ...
    CONJ_TAC THENL [ASM SET_TAC[]; CONV_TAC "SYM_CONV" SYM_CONV] THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN
...
let COMPACT_UNIFORMLY_EQUICONTINUOUS = prove
 (`!(fs:(real^M->real^N)->bool) s.
  ...
  ASM_REWRITE_TAC[] THEN CONV_TAC "NORM_ARITH" NORM_ARITH);;
...
let TRANSLATION_UNIV = prove
 (`!a. IMAGE (\x. a + x) (:real^N) = (:real^N)`,
  CONV_TAC "(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN GEOM_TRANSLATE_TAC[]);;
 ...
 (`!s:real^M->bool a b:real^N.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC CONTINUOUS_ON_ADD THEN REWRITE_TAC[CONTINUOUS_ON_CONST] THEN
    ...
    CONJ_TAC THENL [CONV_TAC "NORM_ARITH" NORM_ARITH; REWRITE_TAC[IN_IMAGE]] THEN
...
let HOMEOMORPHIC_SYM = prove
 (`!s t. s homeomorphic t <=> t homeomorphic s`,
  ...
  REPEAT(AP_TERM_TAC THEN ABS_TAC) THEN CONV_TAC "TAUT" TAUT);;
...
let INJECTIVE_MAP_OPEN_IFF_CLOSED = prove
 (`!f:real^M->real^N s t.
  ...
  CONJ_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC HOMEOMORPHISM_INJECTIVE_OPEN_MAP_EQ;
...
let HOMEOMORPHIC_OPEN_INTERVAL_UNIV_1 = prove
 (`!a b. drop a < drop b ==> interval(a,b) homeomorphic (:real^1)`,
  ...
  ASM_REWRITE_TAC[DROP_VEC; DROP_NEG] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!a:real^N r. &0 < r ==> ball(a,r) homeomorphic (:real^N)`,
  ...
  REPEAT CONJ_TAC THENL
   [X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN DISJ1_TAC THEN
    ...
    REWRITE_TAC[GSYM REAL_INV_MUL; REAL_SUB_0; REAL_INV_EQ_1] THEN
    MP_TAC(ISPEC `y:real^N` NORM_POS_LE) THEN CONV_TAC "REAL_FIELD" REAL_FIELD;
   ...
   [REPEAT STRIP_TAC THEN ASM_CASES_TAC `s:real^M->bool = {}` THEN
    ...
    REWRITE_TAC[GSYM MEMBER_NOT_EMPTY] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[SUBSET; IN_CBALL; IN_UNION; IN_DIFF; IN_BALL; IN_UNIV] THEN
      ...
      UNDISCH_TAC `&0 < e` THEN CONV_TAC "NORM_ARITH" NORM_ARITH;
      EXISTS_TAC `(f:real^M->real^N) x` THEN
 ...
 (`!f:real^N->real^N s.
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    REWRITE_TAC[DOT_SYM] THEN CONV_TAC "REAL_RING" REAL_RING;
...
let MATRIX_WLOG_INVERTIBLE = prove
 (`!P. (!A:real^N^N. invertible A ==> P A) /\
  ...
  ANTS_TAC THENL [ALL_TAC; CONV_TAC "TAUT" TAUT] THEN
 ...
 (`!A:real^N^M B:real^M^N. det(mat 1 + A ** B) = det(mat 1 + B ** A)`,
  ...
  let lemma2 = prove
   (`!A:real^N^M B:real^M^N.
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_EQ_GENERAL THEN
  ...
  THENL [ALL_TAC; CONV_TAC "SYM_CONV" SYM_CONV] THEN
...
let SUMMABLE_IFF_EVENTUALLY = prove
 (`!f g k. (?N. !n. N <= n /\ n IN k ==> f n = g n)
  ...
  REPEAT STRIP_TAC THENL [ALL_TAC; CONV_TAC "SYM_CONV" SYM_CONV] THEN
...
let SUMMABLE_IMP_TOZERO = prove
 (`!f:num->real^N k.
  ...
  COND_CASES_TAC THEN ASM_REWRITE_TAC[NORM_0] THEN CONV_TAC "NORM_ARITH" NORM_ARITH);;
...
let SERIES_INJECTIVE_IMAGE = prove
 (`!x:num->real^N s f l.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[sums] THEN
...
let CLOSEST_POINT_EXISTS = prove
 (`!s a. closed s /\ ~(s = {})
  ...
  REWRITE_TAC[closest_point] THEN CONV_TAC "(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
...
let SETDIST_TRIANGLE = prove
 (`!s a t:real^N->bool.
  ...
  ASM_SIMP_TAC[SETDIST_LE_DIST] THEN CONV_TAC "NORM_ARITH" NORM_ARITH);;
 ...
 (`(!a b:real^N r s.
  ...
  REWRITE_TAC[GSYM REAL_LE_ANTISYM] THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    CONV_TAC "NORM_ARITH" NORM_ARITH] THEN
  MATCH_MP_TAC REAL_LE_TRANS THEN
...
let HAUSDIST_ALT = prove
 (`!s t:real^N->bool.
  ...
  UNDISCH_TAC `dist(y:real^N,x) <= b` THEN CONV_TAC "NORM_ARITH" NORM_ARITH);;
...
let CLOSURE_DYADIC_RATIONALS = prove
 (`closure { inv(&2 pow n) % x |n,x|
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN MATCH_MP_TAC MONO_EXISTS THEN
