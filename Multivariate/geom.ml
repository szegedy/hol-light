   ...
   [MATCH_MP_TAC CONTINUOUS_WITHIN_COMPOSE THEN CONJ_TAC THENL
    ...
    MP_TAC THENL
     [REWRITE_TAC[EVENTUALLY_AND] THEN CONJ_TAC THENL
      ...
      REWRITE_TAC[] THEN (CONV_TAC "NORM_ARITH") NORM_ARITH;
      MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ] EVENTUALLY_MONO) THEN
...
let ARG_EQ_VECTOR_ANGLE_1 = prove
 (`!z. ~(z = Cx(&0)) /\ &0 <= Im z ==> Arg z = vector_angle z (Cx(&1))`,
  ...
  ASM_SIMP_TAC[COMPLEX_NORM_ZERO; REAL_FIELD
   `~(z = &0) ==> (z * x) / z = x`] THEN
  (CONV_TAC "SYM_CONV") SYM_CONV THEN MATCH_MP_TAC ACS_COS THEN
 ...
 (`!w z. ~(w = Cx(&0)) /\ ~(z = Cx(&0))
  ...
  REPEAT STRIP_TAC THEN COND_CASES_TAC THENL
   [SUBGOAL_THEN `z = w * (z / w) /\ w = w * Cx(&1)` MP_TAC THENL
     [REPEAT(POP_ASSUM MP_TAC) THEN (CONV_TAC "COMPLEX_FIELD") COMPLEX_FIELD; ALL_TAC];
    SUBGOAL_THEN `w = z * (w / z) /\ z = z * Cx(&1)` MP_TAC THENL
     [REPEAT(POP_ASSUM MP_TAC) THEN (CONV_TAC "COMPLEX_FIELD") COMPLEX_FIELD; ALL_TAC]] THEN
  DISCH_THEN(fun th -> GEN_REWRITE_TAC (LAND_CONV o ONCE_DEPTH_CONV) [th]) THEN
  ASM_SIMP_TAC[VECTOR_ANGLE_COMPLEX_LMUL] THENL
   [ONCE_REWRITE_TAC[VECTOR_ANGLE_SYM] THEN (CONV_TAC "SYM_CONV") SYM_CONV THEN
    ...
    REWRITE_TAC[COMPLEX_INV_DIV] THEN (CONV_TAC "SYM_CONV") SYM_CONV THEN
...
let AREA_TRIANGLE_SIN = prove
 (`!a b c:real^2.
  ...
  REWRITE_TAC[NORM_POW_2; DOT_2] THEN (CONV_TAC "REAL_RING") REAL_RING);;
