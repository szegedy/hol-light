...
let COPLANAR = prove
 (`2 <= dimindex(:N)
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[LEFT_AND_EXISTS_THM] THEN
  ...
  CONV_TAC "(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN
  REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
...
let COPLANAR_DET_EQ_0 = prove
 (`!v0 v1 (v2: real^3) v3.
  ...
  CONV_TAC "(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
 ...
 (`!f:real^M->real^N sgn s t v.
  ...
  let lemma1 = prove
   (`!f:real^M->real^N s.
    ...
    SUBST1_TAC THENL [ASM SET_TAC[]; CONV_TAC "SYM_CONV" SYM_CONV] THEN
...
let AFFSIGN_SPECIAL_SCALE = prove
 (`!sgn s t a v.
  ...
  DISCH_THEN(CONV_TAC "o" o RAND_CONV o EXPAND_QUANTS_CONV) THEN
...
let AFF_GE_SCALE_LEMMA = prove
 (`!a u v:real^N.
  ...
  UNDISCH_TAC `&0 < a` THEN CONV_TAC "REAL_FIELD" REAL_FIELD);;
...
let ANGLES_ADD_AFF_GE = prove
 (`!u v w x:real^N.
  ...
  UNDISCH_TAC `&0 < a + b` THEN CONV_TAC "REAL_FIELD" REAL_FIELD);;
...
let ORTHONORMAL_CROSS = prove
 (`!e1 e2 e3.
  ...
  CONJ_TAC THENL [CONV_TAC "REAL_RING" REAL_RING; ALL_TAC] THEN
 ...
 (`!v w w1 w2.
  ...
  THENL
   [ASM_SIMP_TAC[COLLINEAR_LEMMA; VECTOR_MUL_EQ_0; REAL_LT_IMP_NZ;
    ...
    CONV_TAC "(BINOP_CONV(BINOP_CONV(ONCE_DEPTH_CONV SYM_CONV)))" (BINOP_CONV(BINOP_CONV(ONCE_DEPTH_CONV SYM_CONV))) THEN
...
let AZIM_UNIQUE = prove
 (`!v w w1 w2 h1 h2 r1 r2 e1 e2 e3 psi theta.
  ...
  SUBGOAL_THEN `(azim v w w1 w2 - theta) / (&2 * pi) = &0` MP_TAC THENL
   [ALL_TAC; MP_TAC PI_POS THEN CONV_TAC "REAL_FIELD" REAL_FIELD] THEN
 ...
 (`!f. linear f /\ (!x. norm(f x) = norm x) /\
  ...
  ANTS_TAC THENL
   [ASM_MESON_TAC[ORTHOGONAL_TRANSFORMATION;
    ...
    ALL_TAC] THEN
  DISCH_THEN(CONV_TAC "o" o LAND_CONV o EXPAND_QUANTS_CONV) THEN
...
let AZIM_SPECIAL_SCALE = prove
 (`!a v w1 w2.
  ...
  FIRST_ASSUM(MP_TAC o MATCH_MP(MESON[REAL_LT_IMP_NZ; REAL_DIV_LMUL]
   `!a. &0 < a ==> (!y. ?x. a * x = y)`)) THEN
  DISCH_THEN(MP_TAC o MATCH_MP QUANTIFY_SURJECTION_THM) THEN
  DISCH_THEN(CONV_TAC "o" o RAND_CONV o
...
let AZIM_SCALE_ALL = prove
 (`!a v w1 w2.
  ...
  let SCALE_QUANT_TAC side asm avoid =
    ...
    DISCH_THEN(MP_TAC o MATCH_MP QUANTIFY_SURJECTION_THM) THEN
    DISCH_THEN(CONV_TAC "o" o side o PARTIAL_EXPAND_QUANTS_CONV avoid) in
  REPEAT STRIP_TAC THEN
...
let AZIM_ARG = prove
 (`!x y:real^3. azim (vec 0) (basis 3) x y = Arg(dropout 3 y / dropout 3 x)`,
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ARG_UNIQUE THEN
 ...
 (`!v0 v1 w x y.
  ...
  ANTS_TAC THENL
   [ASM_REWRITE_TAC[IN_INSERT; FINITE_INSERT; FINITE_EMPTY; NOT_IN_EMPTY] THEN
    ...
    DISCH_THEN SUBST1_TAC] THEN
  REWRITE_TAC[AZIM_ARG] THEN CONV_TAC "(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
 ...
 (`!v0 v1 w x.
  ...
  ANTS_TAC THENL
   [ASM_REWRITE_TAC[IN_INSERT; FINITE_INSERT; FINITE_EMPTY; NOT_IN_EMPTY] THEN
    ...
    DISCH_THEN SUBST1_TAC] THEN
  REWRITE_TAC[AZIM_ARG] THEN CONV_TAC "(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN
  CONV_TAC "(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN REWRITE_TAC[ARG_EQ_PI] THEN
  MATCH_MP_TAC EQ_TRANS THEN EXISTS_TAC
   ...
   [REPEAT(POP_ASSUM MP_TAC) THEN REWRITE_TAC[COLLINEAR_BASIS_3] THEN
    ...
    EQ_TAC THENL
     [REWRITE_TAC[GSYM real; REAL] THEN
      ...
      CONJ_TAC THENL
       [ALL_TAC; REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "REAL_FIELD" REAL_FIELD];
 ...
 (`!v0 v1 w1 w2.
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `collinear {v0:real^3,v1,w1}` THENL
   [MP_TAC(ISPECL [`v0:real^3`; `v1:real^3`; `w1:real^3`; `w2:real^3`]
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "TAUT" TAUT;
    POP_ASSUM MP_TAC] THEN
  ASM_CASES_TAC `collinear {v0:real^3,v1,w2}` THENL
   [MP_TAC(ISPECL [`v0:real^3`; `v1:real^3`; `w2:real^3`; `w1:real^3`]
                NOT_COPLANAR_NOT_COLLINEAR) THEN
    ASM_REWRITE_TAC[] THEN REWRITE_TAC[INSERT_AC] THEN CONV_TAC "TAUT" TAUT;
    POP_ASSUM MP_TAC] THEN
...
let DIHV_LINEAR_IMAGE = prove
 (`!f:real^M->real^N w0 w1 w2 w3.
  ...
  CONV_TAC "(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN
...
let DIHV_RANGE = prove
 (`!w0 w1 w2 w3. &0 <= dihV w0 w1 w2 w3 /\ dihV w0 w1 w2 w3 <= pi`,
  REWRITE_TAC[DIHV] THEN CONV_TAC "(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN
 ...
 (`!u v w u' e1 e2 e3 r phi theta.
  ...
  MP_TAC THENL
   [ALL_TAC;
    ...
    SIMP_TAC[DOT_BASIS; DIMINDEX_3; ARITH] THEN CONV_TAC "REAL_RING" REAL_RING] THEN
  REPEAT(FIRST_X_ASSUM(MP_TAC o
        ...
        EXISTS_TAC `{z | &0 <= Im z}` THEN CONJ_TAC THENL
         [MATCH_MP_TAC CONTINUOUS_WITHIN_UPPERHALF_ARG THEN
          ASM_CASES_TAC `t = &1` THENL
           [ASM_REWRITE_TAC[REAL_SUB_REFL] THEN CONV_TAC "COMPLEX_RING" COMPLEX_RING;
            ALL_TAC] THEN
     ...
     [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE LAND_CONV [AZIM_ARG]) THEN
      ...
      EQ_TAC THENL
       [DISCH_TAC THEN
        ...
        UNDISCH_TAC `~(v = &0)` THEN CONV_TAC "REAL_FIELD" REAL_FIELD;
        REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
...
let VOLUME_OF_TETRAHEDRON = prove
 (`!v1 v2 v3 v4:real^3.
  ...
  REPEAT GEN_TAC THEN CONV_TAC "(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN
...
let AREA_UNIT_CBALL = prove
 (`measure(cball(vec 0:real^2,&1)) = pi`,
  ...
  REWRITE_TAC[ASN_1; ASN_NEG_1] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
 ...
 (`!z:real^3 r. &0 <= r ==> measure(cball(z,r)) = &4 / &3 * pi * r pow 3`,
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL [ASM_REAL_ARITH_TAC; ALL_TAC] THEN
    ...
    CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN CONV_TAC "REAL_RING" REAL_RING;
...
let VOLUME_FRUSTT_STRONG = prove
 (`!v0 v1:real^3 h a.
  ...
  CONV_TAC "(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN GEOM_ORIGIN_TAC `v0:real^3` THEN
...
let MEASURE_LUNE_DECOMPOSITION = prove
 (`!v1 v2 v3:real^3.
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC MEASURE_BALL_AFF_GT_SHUFFLE THEN
...
let VOLUME_SOLID_TRIANGLE = prove
 (`!r v0 v1 v2 v3:real^3.
  ...
  CONV_TAC "(TOP_DEPTH_CONV let_CONV)" (TOP_DEPTH_CONV let_CONV) THEN REPEAT STRIP_TAC THEN
  SUBGOAL_THEN
 ...
 (`!v0 v1:real^3 w1 w2 h a.
  ...
  CONJ_TAC THENL
   [X_GEN_TAC `t:real` THEN DISCH_TAC THEN REWRITE_TAC[] THEN
    ...
    CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_MUL_RZERO] THEN
 ...
 (`!v0 v1:real^3 w1 w2 r a.
  ...
  THENL
   [ASM_SIMP_TAC[VOLUME_CONIC_CAP_WEDGE_WEAK] THEN
    ...
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  COND_CASES_TAC THENL
