 ...
 (`!x y a u v. x < a /\ y < a /\ &0 <= u /\ &0 <= v /\ (u + v = &1)
  ...
  EXISTS_TAC `u * a + v * a:real` THEN CONJ_TAC THENL
   [ASM_SIMP_TAC[REAL_CONVEX_BOUND2_LT];
    ...
    UNDISCH_TAC `u + v = &1` THEN (CONV_TAC "REAL_RING") REAL_RING]);;
...
let REAL_NON_MONOTONE = prove
 (`!P f:real->real.
  ...
  (CONV_TAC "NNFC_CONV") NNFC_CONV THEN (CONV_TAC "CNF_CONV") CNF_CONV THEN
...
let COUNTABLE_LOCAL_MAXIMA = prove
 (`!f:real->real.
  ...
  STRIP_TAC THEN ASM_REWRITE_TAC[IN] THEN CONJ_TAC THENL
   [(CONV_TAC "SYM_CONV") SYM_CONV THEN MATCH_MP_TAC REAL_SUP_UNIQUE THEN
...
let GENERAL_REDUCTION_THEOREM_2 = prove
 (`!P. P {} /\
  ...
  (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN SIMP_TAC[SUBSET_EMPTY] THEN
  STRIP_TAC THEN
  ...
  (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN REWRITE_TAC[IN_INSERT] THEN STRIP_TAC THEN
   ...
   [SUBGOAL_THEN
    ...
    MP_TAC THENL
     [TRANS_TAC CARD_EQ_TRANS `(:num#num)` THEN CONJ_TAC THENL
      ...
      (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN ASM_MESON_TAC[]];
    ALL_TAC] THEN
     ...
     [SUBGOAL_THEN `((:num) DELETE 0) CROSS ((:num) DELETE 0) =_c (:num)`
      ...
      REPEAT CONJ_TAC THENL
       [REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
        ...
        (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN ASM SET_TAC[];
        ASM SET_TAC[];
        REPEAT STRIP_TAC THEN
        REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
   ...
   [MAP_EVERY X_GEN_TAC [`s:num->num`; `s':num->num->num`] THEN STRIP_TAC THEN
    ...
    STRIP_ASSUME_TAC THENL
     [SUBGOAL_THEN
      ...
      REWRITE_TAC[LIST_OF_SEQ_EQ_NIL] THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
