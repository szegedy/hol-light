...
let FRECHET_DERIVATIVE_WORKS = prove
 (`!f net. f differentiable net <=>
  ...
  (CONV_TAC "(RAND_CONV SELECT_CONV)") (RAND_CONV SELECT_CONV) THEN REWRITE_TAC[differentiable]);;
...
let HAS_FRECHET_DERIVATIVE_UNIQUE_AT = prove
 (`!f:real^M->real^N f' x.
  ...
  ASM_REWRITE_TAC[frechet_derivative] THEN (CONV_TAC "SELECT_CONV") SELECT_CONV THEN
  ASM_MESON_TAC[]);;
 ...
 (`!f:real^M->real^N g:real^M->real^P f' g' x.
  ...
  MP_TAC THENL
   [FIRST_X_ASSUM(CONJUNCTS_THEN(MP_TAC o CONJUNCT2 o
    ...
    REWRITE_TAC[NORM_LIFT] THEN (CONV_TAC "NORM_ARITH") NORM_ARITH;
 ...
 (`!f:real^M->real^N f' g g' s a.
  ...
  CONJ_TAC THENL
   [RULE_ASSUM_TAC(REWRITE_RULE[FUN_EQ_THM; o_THM; I_THM]) THEN
    ASM_SIMP_TAC[LINEAR_SUB] THEN (CONV_TAC "NORM_ARITH") NORM_ARITH;
    ALL_TAC] THEN
 ...
 (`!f:real^M->real^N g f' g' t y.
  ...
  STRIP_ASSUME_TAC THENL
   [EXISTS_TAC `&2 * C` THEN
    ...
    (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[NORM_TRIANGLE_SUB];
    ALL_TAC] THEN
   ...
   [X_GEN_TAC `e0:real` THEN REPEAT STRIP_TAC THEN
    ...
    TRANS_TAC REAL_LTE_TRANS `(e:real) pow 2` THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_ARITH `a / &2 / b * x < a <=> a * x / b < a * &2`] THEN
      ...
      UNDISCH_TAC `&0 < l` THEN (CONV_TAC "NORM_ARITH") NORM_ARITH;
   ...
   [REPEAT STRIP_TAC THEN REMOVE_THEN "L2" (MP_TAC o SPEC `x:real^N`) THEN
    ...
    CONJ_TAC THENL
     [RULE_ASSUM_TAC(REWRITE_RULE[FUN_EQ_THM; o_THM; I_THM]) THEN
      ...
      AP_TERM_TAC THEN (CONV_TAC "VECTOR_ARITH") VECTOR_ARITH;
 ...
 (`!f:real^N->real^N f' s.
  ...
  MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
   [TRANS_TAC SUBSET_TRANS `ball(a:real^N,&2 * r)` THEN
    ...
    DISCH_TAC] THEN
  X_GEN_TAC `b:real^N` THEN DISCH_TAC THEN (CONV_TAC "SYM_CONV") SYM_CONV THEN
  ...
  MP_TAC THENL
   [REPEAT GEN_TAC THEN STRIP_TAC THEN REWRITE_TAC[IN_BALL] THEN
    ...
    MATCH_MP_TAC(REAL_ARITH `y <= x /\ &0 <= y - x + &1
      ==> abs(x - y) <= &1`) THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_MUL2 THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
         ...
         [MATCH_MP_TAC ALWAYS_EVENTUALLY THEN
          ...
          ASM_REWRITE_TAC[] THENL
           [REWRITE_TAC[VECTOR_SUB_REFL] THEN
            ...
            BINOP_TAC THENL [ALL_TAC; (CONV_TAC "VECTOR_ARITH") VECTOR_ARITH] THEN
     ...
     [DISCH_THEN(fun th -> MATCH_MP_TAC LIM_MATRIX_INV THEN MP_TAC th) THEN
      ...
      CONJ_TAC THENL
       [ALL_TAC;
        ...
        X_GEN_TAC `y:real^N` THEN STRIP_TAC THEN (CONV_TAC "SYM_CONV") SYM_CONV] THEN
...
let VECTOR_DERIVATIVE_WORKS = prove
 (`!net f:real^1->real^N.
  ...
  (CONV_TAC "(RAND_CONV SELECT_CONV)") (RAND_CONV SELECT_CONV) THEN
...
let HAS_VECTOR_DERIVATIVE_UNIQUE_AT = prove
 (`!f:real^1->real^N f' x.
  ...
  ASM_REWRITE_TAC[vector_derivative] THEN (CONV_TAC "SELECT_CONV") SELECT_CONV THEN
  ASM_MESON_TAC[]);;
