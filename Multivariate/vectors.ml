...
let TRANSLATION_SUBSET_GALOIS_RIGHT = prove
 (`!s t a:real^N.
  ...
  REWRITE_TAC[] THEN CONV_TAC "VECTOR_ARITH" VECTOR_ARITH);;
...
let SQRT_WORKS_GEN = prove
 (`!x. real_sgn(sqrt x) = real_sgn x /\ sqrt(x) pow 2 = abs x`,
  GEN_TAC THEN REWRITE_TAC[sqrt] THEN CONV_TAC "SELECT_CONV" SELECT_CONV THEN
 ...
 (`!x:real^N.
  ...
  let lemma = prove
   (`!x n. &n * sum(1..n) (\i. x i pow 2) - (sum(1..n) x) pow 2 =
    ...
    GEN_TAC THEN CONV_TAC "(BINDER_CONV SYM_CONV)" (BINDER_CONV SYM_CONV) THEN INDUCT_TAC THEN
...
let NORM_LT_SQUARE_ALT = prove
 (`!x:real^N. norm(x) < a <=> &0 <= a /\ x dot x < a pow 2`,
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `a = &0` THENL
   [ASM_REWRITE_TAC[real_ge] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let NORM_ARITH_TAC = CONV_TAC "NORM_ARITH" NORM_ARITH;;
...
let VSUM_SUPPORT = prove
 (`!f:A->real^N s. vsum {x | x IN s /\ ~(f x = vec 0)} f = vsum s f`,
  REPEAT GEN_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_SUPERSET THEN
  SET_TAC[]);;
...
let NORM_VSUM_PYTHAGOREAN = prove
 (`!k u:A->real^N.
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[PAIRWISE_INSERT] THEN
...
let ADJOINT_WORKS = prove
 (`!f:real^M->real^N. linear f ==> !x y. f(x) dot y = x dot (adjoint f)(y)`,
  GEN_TAC THEN DISCH_TAC THEN SIMP_TAC[adjoint] THEN CONV_TAC "SELECT_CONV" SELECT_CONV THEN
...
let ADJOINT_COMPOSE = prove
 (`!f g:real^N->real^N.
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ADJOINT_UNIQUE THEN
...
let ORTHOGONAL_PROJECTION_EQ_SELF_ADJOINT_IDEMPOTENT = prove
 (`!f:real^N->real^N.
  ...
  EQ_TAC THENL
   [REPEAT STRIP_TAC THENL
     [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ADJOINT_UNIQUE THEN
...
let ADJOINT_MATRIX = prove
 (`!A:real^N^M. adjoint(\x. A ** x) = (\x. transp A ** x)`,
  GEN_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC ADJOINT_UNIQUE THEN
 ...
 (`!p. dependent (p:real^N -> bool) <=>
  ...
  EQ_TAC THEN REWRITE_TAC[LEFT_IMP_EXISTS_THM] THENL
   [MAP_EVERY X_GEN_TAC [`a:real^N`; `s:real^N->bool`; `u:real^N->real`] THEN
    ...
    CONJ_TAC THENL [ASM SET_TAC[]; CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV] THEN
...
let BASIS_EXISTS = prove
 (`!v. ?b. b SUBSET v /\ independent b /\ v SUBSET (span b) /\
           b HAS_SIZE (dim v)`,
  GEN_TAC THEN REWRITE_TAC[dim] THEN CONV_TAC "SELECT_CONV" SELECT_CONV THEN
 ...
 (`!f:real^M->real^N s.
  ...
  FIRST_ASSUM MATCH_MP_TAC THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC SPAN_VSUM THEN ASM_REWRITE_TAC[] THEN
    ...
    FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
    W(MP_TAC o PART_MATCH (lhs o rand) VSUM_IMAGE o lhand o snd) THEN
  ...
  MP_TAC THENL
   [GEN_TAC THEN DISCH_TAC THEN
    MATCH_MP_TAC(TAUT `a /\ (a ==> b) ==> a /\ b`) THEN CONJ_TAC THENL
     [EXPAND_TAC "h" THEN CONV_TAC "SELECT_CONV" SELECT_CONV THEN
  ...
  REPEAT CONJ_TAC THENL
   [MAP_EVERY X_GEN_TAC [`x:real^M`; `y:real^M`] THEN STRIP_TAC THEN
    SUBGOAL_THEN `(h:real^M->real)(x + y) = h(x) + h(y)` ASSUME_TAC THENL
     [CONV_TAC "SYM_CONV" SYM_CONV THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
 ...
 (`!s t:real^N->bool.
  ...
  EQ_TAC THEN DISCH_TAC THEN REPEAT GEN_TAC THEN STRIP_TAC THENL
   [ONCE_REWRITE_TAC[GSYM VECTOR_SUB_EQ] THEN
    ...
    FIRST_X_ASSUM(MP_TAC o SYM) THEN CONV_TAC "VECTOR_ARITH" VECTOR_ARITH;
...
let SPECIAL_HYPERPLANE_SPAN = prove
 (`!k. 1 <= k /\ k <= dimindex(:N)
  ...
  REPEAT STRIP_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SPAN_SUBSPACE THEN
 ...
 (`!f:real^M->real^N s.
  ...
  SUBST1_TAC THENL
   [MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC VSUM_EQ_SUPERSET THEN
  ...
  and lemma_uniqueness = prove
   (`!A:real^M^N u v y.
    ...
    (SUBGOAL_THEN
     ...
     THENL
      [SIMP_TAC[MATRIX_VECTOR_MUL_SUB_LDISTRIB] THEN CONV_TAC "VECTOR_ARITH" VECTOR_ARITH;
...
let MATRIX_INV_UNIQUE_STRONG = prove
 (`!A:real^M^N X.
  ...
  POP_ASSUM(K ALL_TAC) THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
  SUBGOAL_THEN
...
let ORTHOGONAL_TO_ORTHOGONAL_2D = prove
 (`!x y z:real^2.
  ...
  REWRITE_TAC[DOT_2] THEN CONV_TAC "REAL_RING" REAL_RING);;
...
let COLLINEAR_3_2D = prove
 (`!x y z:real^2. collinear{x,y,z} <=>
  ...
  REWRITE_TAC[DOT_2; VECTOR_SUB_COMPONENT] THEN CONV_TAC "REAL_RING" REAL_RING);;
...
let BETWEEN_CMUL_LIFT = prove
 (`!a b c v:real^N.
  ...
  REWRITE_TAC[GSYM NORM_EQ_0] THEN CONV_TAC "REAL_RING" REAL_RING);;
...
let ORTHOGONAL_SCALING = prove
 (`!c. &0 < c ==> !x y. orthogonal (c % x) (c % y) <=> orthogonal x y`,
  REWRITE_TAC[orthogonal; DOT_LMUL; DOT_RMUL] THEN CONV_TAC "REAL_FIELD" REAL_FIELD) in
...
let QUANTIFY_SURJECTION_HIGHER_THM = prove
 (`!f:A->B.
  ...
  GEN_TAC THEN DISCH_TAC THEN CONV_TAC "(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
