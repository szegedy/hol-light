...
let BOUNDED_FUNCTIONS_BIJECTIONS_1 = prove
 (`!p. p IN {(y,g) | y IN s /\
  ...
  (CONV_TAC "(REDEPTH_CONV GEN_BETA_CONV)") (REDEPTH_CONV GEN_BETA_CONV) THEN REWRITE_TAC[IN_ELIM_THM] THEN
...
let FINITE_BOUNDED_FUNCTIONS = prove
 (`!s k. FINITE s
  ...
  PURE_ONCE_REWRITE_TAC[CONJ_SYM] THEN (CONV_TAC "(RAND_CONV SYM_CONV)") (RAND_CONV SYM_CONV) THEN
 ...
 (`!A:real^N^N. invertible(A) <=> ~(det A = &0)`,
  GEN_TAC THEN EQ_TAC THENL
   [REWRITE_TAC[INVERTIBLE_RIGHT_INVERSE; LEFT_IMP_EXISTS_THM] THEN
    ...
    REWRITE_TAC[DET_MUL; DET_I] THEN (CONV_TAC "REAL_RING") REAL_RING;
    ALL_TAC] THEN
...
let INVERTIBLE_NEARBY_ONORM = prove
 (`!A B:real^N^N.
  ...
  (CONV_TAC "(LAND_CONV SYM_CONV)") (LAND_CONV SYM_CONV) THEN REWRITE_TAC[MATRIX_VECTOR_MUL_LID] THEN
...
let DET_2 = prove
 (`!A:real^2^2. det A = A$1$1 * A$2$2 - A$1$2 * A$2$1`,
  ...
  (CONV_TAC "(LAND_CONV(RATOR_CONV(ONCE_DEPTH_CONV NUMSEG_CONV)))") (LAND_CONV(RATOR_CONV(ONCE_DEPTH_CONV NUMSEG_CONV))) THEN
 ...
 (`!A:real^N^N. abs(det A) <= product(1..dimindex(:N)) (\i. norm(row i A))`,
  ...
  ASSUME_TAC THENL
   [ONCE_REWRITE_TAC[SWAP_FORALL_THM] THEN
    ...
    MATCH_MP_TAC num_WF THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
     ...
     [REWRITE_TAC[SUM_DELTA; IN_NUMSEG] THEN COND_CASES_TAC THEN
      ...
      COND_CASES_TAC THEN ASM_REWRITE_TAC[] THENL
       [ASM_CASES_TAC `(b:num->real^N) m = vec 0` THEN
        ...
        (CONV_TAC "SYM_CONV") SYM_CONV THEN REWRITE_TAC[REAL_ENTIRE] THEN
...
let ORTHOGONAL_MATRIX_ORTHONORMAL_ROWS_INDEXED = prove
 (`!A:real^N^N.
  ...
  (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[orthogonal] THEN
  MESON_TAC[]);;
 ...
 (`!A:real^N^N.
  ...
  EQ_TAC THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THENL
   [MATCH_MP_TAC(SET_RULE `UNIV SUBSET s ==> s = UNIV`) THEN
    ...
    (CONV_TAC "SYM_CONV") SYM_CONV THEN REWRITE_TAC[GSYM DIM_UNIV] THEN
...
let ORTHOGONAL_MATRIX_2_ALT = prove
 (`!A:real^2^2. orthogonal_matrix A <=>
                ...
                (A$1$1 = A$2$2 /\ A$1$2 = --(A$2$1) \/
                 A$1$1 = --(A$2$2) /\ A$1$2 = A$2$1)`,
  REWRITE_TAC[ORTHOGONAL_MATRIX_2] THEN (CONV_TAC "REAL_RING") REAL_RING);;
...
let ORTHOGONAL_TRANSFORMATION_ORTHOGONAL_EIGENVECTORS = prove
 (`!f:real^N->real^N v w a b.
  ...
  UNDISCH_TAC `~(a:real = b)` THEN (CONV_TAC "REAL_RING") REAL_RING);;
 ...
 (`!f:real^N->real^N.
  ...
  let lemma = prove
   (`!x:real^N y:real^N x':real^N y':real^N x0 y0 x0' y0'.
    ...
    REWRITE_TAC[DOT_SYM] THEN (CONV_TAC "REAL_RING") REAL_RING) in
  REPEAT STRIP_TAC THEN
...
let REFLECT_ALONG_INVOLUTION = prove
 (`!v x:real^N. reflect_along v (reflect_along v x) = x`,
  ...
  REWRITE_TAC[DOT_LMUL; GSYM DOT_EQ_0] THEN (CONV_TAC "REAL_FIELD") REAL_FIELD);;
   ...
   [REWRITE_TAC[CONJUNCT1 LE; LENGTH_EQ_NIL; ADD_CLAUSES; UNWIND_THM2] THEN
    ...
    ANTS_TAC THENL
     [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (ARITH_RULE
      ...
      (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN ASM_REWRITE_TAC[VECTOR_SUB_EQ] THEN
...
let GEN_GEOM_ORIGIN_TAC x avoid (asl,w as gl) =
  ...
  (MAP_EVERY X_GEN_TAC avs THEN
   ...
   SPEC_TAC(x,x) THEN (CONV_TAC "(GEOM_ORIGIN_CONV avoid)") (GEOM_ORIGIN_CONV avoid)) gl;;
...
let GEOM_TRANSLATE_TAC avoid (asl,w) =
  ...
  (X_GEN_TAC a THEN
   (CONV_TAC "(funpow n BINDER_CONV (LAND_CONV(GEOM_TRANSLATE_CONV avoid a)))") (funpow n BINDER_CONV (LAND_CONV(GEOM_TRANSLATE_CONV avoid a))) THEN
   REWRITE_TAC[]) (asl,w);;
...
let GEOM_TRANSFORM_TAC =
  ...
  fun avoid (asl,w as gl) ->
    ...
    (MAP_EVERY X_GEN_TAC (f::vs) THEN DISCH_TAC THEN
     ...
     (CONV_TAC "(LAND_CONV vconv)") (LAND_CONV vconv) THEN
