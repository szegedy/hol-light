   ...
   [CONJ_TAC THENL
    ...
    STRIP_ASSUME_TAC THENL
     [ASM_REWRITE_TAC[IN_DIFF] THEN ONCE_ASM_REWRITE_TAC[] THEN
      ...
      UNDISCH_TAC `u < &1` THEN CONV_TAC "REAL_FIELD" REAL_FIELD;
  ...
  ASM_CASES_TAC `sum (s DIFF f:real^N->bool) c = &0` THENL
   [SUBGOAL_THEN `!x:real^N. x IN (s DIFF f) ==> c x = &0` MP_TAC THENL
    ...
    ASM_SIMP_TAC[] THEN CONJ_TAC THEN FIRST_X_ASSUM(fun th g ->
      (GEN_REWRITE_TAC RAND_CONV [GSYM th] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
 ...
 (`!s f:real^N->bool.
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[VECTOR_ARITH `&2 % a - b:real^N = a + &1 % (a - b)`] THEN
    ...
    EXISTS_TAC `&1 / &2` THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    VECTOR_ARITH_TAC]);;
 ...
 (`!s f:real^N->bool. conic s /\ f face_of s ==> conic f`,
  ...
  MP_TAC THENL
   [FIRST_X_ASSUM(DISJ_CASES_TAC o MATCH_MP (REAL_ARITH
    ...
    UNDISCH_TAC `d:real < e` THEN CONV_TAC "REAL_FIELD" REAL_FIELD]);;
...
let EMPTY_EXPOSED_FACE_OF = prove
 (`!s:real^N->bool. {} exposed_face_of s`,
  ...
  REWRITE_TAC[DOT_LZERO] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN SET_TAC[]);;
...
let EXTREME_POINT_NOT_IN_RELATIVE_INTERIOR = prove
 (`!s x:real^N.
  ...
  REPEAT GEN_TAC THEN CONV_TAC "(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
 ...
 (`!s a b:real^N.
  ...
  MAP_EVERY EXISTS_TAC [`b:real^N`; `(b:real^N) dot b`] THEN CONJ_TAC THENL
   [X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN ASM_REWRITE_TAC[VECTOR_EQ] THEN
 ...
 (`!u s:real^N->bool.
  ...
  ASM_CASES_TAC `(vec 0:real^N) IN affine hull u` THENL
   [SUBGOAL_THEN
    ...
    SIMP_TAC[SUM_CLAUSES] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV]);;
...
let FACET_OF_EMPTY = prove
 (`!s. ~(s facet_of {})`,
  REWRITE_TAC[facet_of; FACE_OF_EMPTY] THEN CONV_TAC "TAUT" TAUT);;
...
let EXTREME_POINTS_OF_CONVEX_HULL_EQ = prove
 (`!s:real^N->bool.
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC KREIN_MILMAN_MINKOWSKI THEN
 ...
 (`!f s a:real^N.
  ...
  let lemma = prove
   (`!a b c p:real^N u v w x.
    ...
    UNDISCH_TAC `~(x = &0)` THEN CONV_TAC "REAL_FIELD" REAL_FIELD) in
  REPEAT STRIP_TAC THEN EQ_TAC THENL
   ...
   [ASM_REWRITE_TAC[VECTOR_MUL_LZERO; VECTOR_ADD_LID] THEN
    ...
    REWRITE_TAC[REAL_MUL_LZERO; REAL_MUL_LID] THEN REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LT_MUL THEN ASM_REAL_ARITH_TAC;
      ...
      UNDISCH_TAC `~(ux = &0)` THEN CONV_TAC "REAL_FIELD" REAL_FIELD];
    ...
    MATCH_MP_TAC(TAUT `p ==> ~p ==> q`) THEN
    CONJ_TAC THENL [CONV_TAC "REAL_RING" REAL_RING; REPEAT CONJ_TAC] THEN
 ...
 (`!s a b c:real^N.
  ...
  SUBGOAL_THEN
   `inv(u - w * u + w * v) * (u - w * u) =
    &1 - inv(u - w * u + w * v) * w * v`
  SUBST1_TAC THENL
   [UNDISCH_TAC `&0 < u - w * u + w * v` THEN CONV_TAC "REAL_FIELD" REAL_FIELD;
   ...
   (`!a x:real^N s.
    ...
    STRIP_ASSUME_TAC THENL
     [EXISTS_TAC `setdist({vec 0:real^N},s) / a - norm(x:real^N)` THEN
      ...
      ASM_SIMP_TAC[REAL_LE_LMUL_EQ] THEN CONV_TAC "NORM_ARITH" NORM_ARITH;
      MATCH_MP_TAC(MESON[]
      ...
      REWRITE_TAC[IN_BALL] THEN CONV_TAC "NORM_ARITH" NORM_ARITH]) in
  let flemma = prove
   (`!a x:real^N s.
       convex s /\ bounded s /\ ~(vec 0 IN closure s) /\
       &0 < a /\ ~(x = vec 0) /\ ~(s = {}) /\
...
let CONVEX_SYMDIFF_CLOSE_TO_FRONTIER = prove
 (`!s t:real^N->bool e.
  ...
  ASM_REWRITE_TAC[IN_BALL_0; GSYM dist] THEN CONV_TAC "VECTOR_ARITH" VECTOR_ARITH);;
...
let POLYTOPE_AFF_DIM_1 = prove
 (`!p:real^N->bool.
  ...
  ASM_REWRITE_TAC[AFF_DIM_EMPTY] THEN CONV_TAC "INT_REDUCE_CONV" INT_REDUCE_CONV THEN
  DISCH_TAC THEN
 ...
 (`!s:real^N->bool e.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUBSET_SUMS_RCANCEL THEN
    ...
    ASM_REWRITE_TAC[GSYM dist] THEN CONV_TAC "VECTOR_ARITH" VECTOR_ARITH;
    FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (REAL_ARITH
     `&0 < e ==> x <= e / &2 ==> x < e`)) THEN
    MATCH_MP_TAC REAL_HAUSDIST_LE THEN
    ASM_REWRITE_TAC[CONVEX_HULL_EQ_EMPTY] THEN CONJ_TAC THENL
   ...
   [REPEAT STRIP_TAC THEN REWRITE_TAC[facet_of] THEN CONJ_TAC THENL
    ...
    MATCH_MP_TAC(TAUT `b /\ (b ==> a) ==> a /\ b`) THEN CONJ_TAC THENL
     [EXPAND_TAC "c" THEN REWRITE_TAC[DOT_RADD; DOT_RMUL] THEN
      ASM_REWRITE_TAC[] THEN CONV_TAC "REAL_RING" REAL_RING;
      DISCH_TAC] THEN
...
let POLYHEDRON_NEGATIONS = prove
 (`!s:real^N->bool. polyhedron s ==> polyhedron(IMAGE (--) s)`,
  GEN_TAC THEN MATCH_MP_TAC EQ_IMP THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
 ...
 (`!s:real^N->bool.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC CONVEX_CONE_MUL THEN
    ...
    ASM_SIMP_TAC[REAL_LT_IMP_LE] THEN CONV_TAC "NORM_ARITH" NORM_ARITH;
    REWRITE_TAC[VECTOR_ADD_LDISTRIB; VECTOR_MUL_ASSOC] THEN
 ...
 (`!s:real^N->bool.
  ...
  THENL
   [ASM_SIMP_TAC[CARD_EQ_0; CONVEX_HULL_EMPTY] THEN SET_TAC[];
    ...
    REPEAT STRIP_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[EMPTY_UNIONS] THEN
...
let POLYTOPE_FACET_LOWER_BOUND = prove
 (`!p:real^N->bool.
  ...
  CONV_TAC "INT_REDUCE_CONV" INT_REDUCE_CONV THEN STRIP_TAC THEN
  SUBGOAL_THEN
     ...
     [MATCH_MP_TAC(TAUT
      ...
      REPEAT CONJ_TAC THENL
       [MESON_TAC[POLYTOPE_IMP_CONVEX; POLYTOPE_IMP_CLOSED];
        ASM SET_TAC[];
        REPEAT STRIP_TAC THEN CONV_TAC "SELECT_CONV" SELECT_CONV THEN
 ...
 (`!s u:real^N->bool.
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[GSYM UNIONS_UNION] THEN EXPAND_TAC "u" THEN AP_TERM_TAC THEN
    SET_TAC[];
    ALL_TAC] THEN
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC lemma THEN
