...
let REAL_OPEN_IN = prove
 (`!s. real_open s <=> open_in euclideanreal s`,
  GEN_TAC THEN REWRITE_TAC[euclideanreal] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
...
let REALLIM_NULL_NEG = prove
 (`!net f. ((\x. --(f x)) ---> &0) net <=> (f ---> &0) net`,
  ...
  MATCH_MP_TAC REALLIM_NULL_LMUL_EQ THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV);;
...
let TRIVIAL_LIMIT_WITHINREAL_WITHINCOMPLEX = prove
 (`trivial_limit(atreal x within s) <=>
  ...
  REPEAT CONJ_TAC THEN TRY EQ_TAC THEN REWRITE_TAC[LEFT_IMP_EXISTS_THM] THENL
   [DISCH_THEN(MP_TAC o SPECL [`&0`; `&1`]) THEN CONV_TAC "REAL_RING" REAL_RING;
 ...
 (`!s a. real_summable s a
  ...
  EXISTS_TAC `&1` THEN REWRITE_TAC[REAL_LT_01] THEN CONJ_TAC THENL
   [REWRITE_TAC[IMP_CONJ; IN_INTER; IN_ELIM_THM; IN_IMAGE] THEN
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM REAL; o_DEF; CX_MUL; CX_POW] THEN
...
let HAS_REAL_DERIVATIVE_FRAC = prove
 (`!x. ~(integer x) ==> (frac has_real_derivative (&1)) (atreal x)`,
  ...
  X_GEN_TAC `y:real` THEN DISCH_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
...
let REAL_MVT = prove
 (`!f f' a b.
  ...
  CONJ_TAC THENL [UNDISCH_TAC `a < b` THEN CONV_TAC "REAL_FIELD" REAL_FIELD; ALL_TAC] THEN
...
let REAL_MVT_CAUCHY = prove
 (`!f g f' g' a b.
  ...
  UNDISCH_TAC `a < b` THEN CONV_TAC "REAL_FIELD" REAL_FIELD);;
...
let REAL_TAYLOR_MVT_POS = prove
 (`!f a x n.
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
    ...
    MATCH_MP_TAC HAS_REAL_DERIVATIVE_ADD THEN CONJ_TAC THENL
     [ALL_TAC;
      REAL_DIFF_TAC THEN REWRITE_TAC[ADD_SUB] THEN CONV_TAC "REAL_RING" REAL_RING] THEN
  ...
  SUBGOAL_THEN `~(&(FACT n) = &0)` MP_TAC THENL
   [REWRITE_TAC[REAL_OF_NUM_EQ; FACT_NZ]; CONV_TAC "REAL_FIELD" REAL_FIELD]);;
 ...
 (`!f f' f'' s.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_CONVEX_ON_DERIVATIVE_INCREASING;
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC HAS_REAL_DERIVATIVE_INCREASING] THEN
  ASM_REWRITE_TAC[]);;
 ...
 (`!a x k:A->bool.
  ...
  MP_TAC THENL
   [MATCH_MP_TAC version2 THEN
    ...
    MATCH_MP_TAC EQ_IMP THEN CONV_TAC "SYM_CONV" SYM_CONV THEN BINOP_TAC THENL
...
let LOG_CONVEX_IMP_POS = prove
 (`!f s x:real^N.
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ASM_MESON_TAC[]);;
...
let REAL_INTEGRAL_HAS_REAL_DERIVATIVE_POINTWISE = prove
 (`!f a b x.
  ...
  REWRITE_TAC[LIFT_DROP] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
...
let REAL_INDEFINITE_INTEGRAL_CONTINUOUS_RIGHT = prove
 (`!f:real->real a b.
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INTEGRAL THEN
  MATCH_MP_TAC REAL_INTEGRABLE_SUBINTERVAL THEN
  MAP_EVERY EXISTS_TAC [`a:real`; `b:real`] THEN
  ASM_REWRITE_TAC[SUBSET_REAL_INTERVAL] THEN
...
let HAS_REAL_DERIVATIVE_INDEFINITE_INTEGRAL = prove
 (`!f a b.
  ...
  REWRITE_TAC[GSYM IMAGE_LIFT_REAL_INTERVAL] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
  MATCH_MP_TAC REAL_INTEGRAL THEN
  MATCH_MP_TAC REAL_INTEGRABLE_SUBINTERVAL THEN
  MAP_EVERY EXISTS_TAC [`a:real`; `b:real`] THEN
  ASM_REWRITE_TAC[SUBSET_REAL_INTERVAL] THEN
...
let REAL_MONOTONE_CONVERGENCE_INCREASING = prove
 (`!f:num->real->real g s.
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN REWRITE_TAC[GSYM o_DEF] THEN
...
let HAS_REAL_MEASURE_REAL_NEGLIGIBLE_UNIONS_IMAGE = prove
 (`!f:A->(real->bool) s.
  ...
  SUBST1_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN ONCE_REWRITE_TAC[GSYM o_DEF] THEN
 ...
 (`!f:A->real->bool s.
  ...
  MATCH_MP_TAC EQ_IMP THEN BINOP_TAC THENL
   [GEN_REWRITE_TAC I [EXTENSION] THEN
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_SUPERSET THEN
...
let REAL_MEASURE_UNION = prove
 (`!s t. real_measurable s /\ real_measurable t
  ...
  (CONJ_TAC THENL
    [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_MEASURE_DISJOINT_UNION THEN
...
let SLICE_CBALL = prove
 (`!k t x r.
  ...
  THENL
   [CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC DROPOUT_PUSHIN THEN ASM_ARITH_TAC;
    ASM_SIMP_TAC[DOT_DROPOUT] THEN
    ...
    REWRITE_TAC[] THEN CONJ_TAC THENL
     [X_GEN_TAC `i:num` THEN COND_CASES_TAC THEN ASM_REWRITE_TAC[] THENL
       [ASM_MESON_TAC[REAL_NOT_LT; IN_NUMSEG; REAL_NOT_LE; INTERVAL_EQ_EMPTY];
        REWRITE_TAC[EMPTY_SUBSET] THEN CONV_TAC "(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN
 ...
 (`!k s:real^N->bool e.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC REAL_INTEGRAL_LE THEN ASM_REWRITE_TAC[] THEN
    ...
    X_GEN_TAC `j:num` THEN STRIP_TAC THEN REWRITE_TAC[] THEN
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC REAL_INTEGRAL_UNIQUE THEN
    SUBGOAL_THEN `?u v. u$k <= v$k /\
                          (d:num->real^N->bool) j = interval[u,v]`
    STRIP_ASSUME_TAC THENL [ASM_MESON_TAC[]; ALL_TAC] THEN
    ASM_REWRITE_TAC[] THEN
       ...
       [ASM_SIMP_TAC[REAL_DIV_REFL; REAL_LT_IMP_NZ; REAL_SUB_LT] THEN
        ...
        SUBST1_TAC THENL
         [REWRITE_TAC[REAL_FRAC_EQ] THEN
          ...
          ASM_REAL_ARITH_TAC;
          AP_TERM_TAC THEN UNDISCH_TAC `a:real < b` THEN CONV_TAC "REAL_FIELD" REAL_FIELD]];
      ASM_SIMP_TAC[REAL_FIELD
   ...
   [SUBGOAL_THEN
    ...
    THENL
     [ASM_REAL_ARITH_TAC;
      ...
      ASM_REWRITE_TAC[REAL_ADD_LID; REAL_LE_REFL] THEN CONV_TAC "REAL_RING" REAL_RING;
      ALL_TAC] THEN
...
let BERNSTEIN_LEMMA = prove
 (`!n x. sum(0..n) (\k. (&k - &n * x) pow 2 * bernstein n k x) =
   ...
   [REPEAT GEN_TAC THEN MATCH_MP_TAC REAL_DERIVATIVE_UNIQUE_ATREAL THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC HAS_REAL_DERIVATIVE_SUM THEN REWRITE_TAC[FINITE_NUMSEG];
      ASM_REWRITE_TAC[]] THEN
    REPEAT STRIP_TAC THEN REAL_DIFF_TAC THEN CONV_TAC "REAL_RING" REAL_RING;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `!x y. sum(0..n)
   ...
   [REPEAT GEN_TAC THEN MATCH_MP_TAC REAL_DERIVATIVE_UNIQUE_ATREAL THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC HAS_REAL_DERIVATIVE_SUM THEN REWRITE_TAC[FINITE_NUMSEG];
      ASM_REWRITE_TAC[]] THEN
    REPEAT STRIP_TAC THEN REAL_DIFF_TAC THEN
    REWRITE_TAC[ARITH_RULE `n - 1 - 1 = n - 2`] THEN CONV_TAC "REAL_RING" REAL_RING;
    ALL_TAC] THEN
  REWRITE_TAC[REAL_ARITH
   `(a - b) pow 2 * x =
  ...
  SUBST1_TAC THENL [ALL_TAC; CONV_TAC "REAL_RING" REAL_RING] THEN
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN ASM_CASES_TAC `y:real^N = x` THENL
     [EXISTS_TAC `\z:real^N. (f:real^N->real) x` THEN ASM_SIMP_TAC[];
      ...
      ASM_SIMP_TAC[] THEN
      UNDISCH_TAC `~((h:real^N->real) x = h y)` THEN CONV_TAC "REAL_FIELD" REAL_FIELD];
      ALL_TAC] THEN
     ...
     [MESON_TAC[];
      ...
      ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
       [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
          FINITE_SUBSET)) THEN
        SET_TAC[];
        ABS_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_EQ_SUPERSET THEN
     ...
     [REPEAT GEN_TAC THEN DISCH_THEN(CONJUNCTS_THEN2
      ...
      REWRITE_TAC[] THEN CONJ_TAC THENL
       [MATCH_MP_TAC FINITE_SUBSET THEN
        ...
        CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_ADD_GEN THEN
 ...
 (`(!n x. sum(0..n) (\k. &(binom(n,k)) * bernoulli k x) - bernoulli n x =
  ...
  let lemma = prove
   (`(!n x. sum (0..n) (\k. &(binom(n,k)) * B k x) - B n x =
    ...
    CONV_TAC "REAL_FIELD" REAL_FIELD) in
...
let BERNOULLI_NUMBER = prove
 (`!n. sum (0..n) (\k. &(binom (n,k)) * bernoulli k (&0)) - bernoulli n (&0) =
  ...
  CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[real_pow; REAL_MUL_LID] THEN
...
let HAS_REAL_INTEGRAL_BERNOULLI = prove
 (`!n. ((bernoulli n) has_real_integral (if n = 0 then &1 else &0))
  ...
  REWRITE_TAC[REAL_POS] THEN ANTS_TAC THENL
   [REPEAT STRIP_TAC THEN REAL_DIFF_TAC THEN
    REWRITE_TAC[ADD_SUB; GSYM REAL_OF_NUM_ADD] THEN CONV_TAC "REAL_FIELD" REAL_FIELD;
 ...
 (`!p n. polynomial_function p /\
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[GSYM INFINITE] THEN
    ...
    CONV_TAC "(BINDER_CONV SYM_CONV)" (BINDER_CONV SYM_CONV) THEN INDUCT_TAC THEN
...
let BERNOULLI_RAABE_2 = prove
 (`!n x. bernoulli n ((x + &1) / &2) + bernoulli n (x / &2) =
  ...
  ASM_REWRITE_TAC[bernoulli] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC POLYNOMIAL_FUNCTION_LMUL THEN
    ...
    CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM IMP_CONJ_ALT] THEN
    ...
    CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ONCE_REWRITE_TAC[CONJ_SYM] THEN
...
let BERNOULLI_1_0 = prove
 (`!n. bernoulli n (&1) = --(&1) pow n * bernoulli n (&0)`,
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_POW_ONE; REAL_MUL_LID]);;
  ...
  let lemma = prove
   (`(!n x. x IN real_interval(&0,&1 / &2)
    ...
    REWRITE_TAC[AND_FORALL_THM; IN_REAL_INTERVAL] THEN INDUCT_TAC THENL
     [CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
      CONV_TAC "(ONCE_DEPTH_CONV BERNOULLI_CONV)" (ONCE_DEPTH_CONV BERNOULLI_CONV) THEN REAL_ARITH_TAC;
      ...
      MP_TAC(ISPECL
       [`\x. bernoulli (2 * SUC n + 1) x / (&2 * &n + &3)`;
        `bernoulli (2 * SUC n)`; `x:real`; `&1 / &2`]
          REAL_ROLLE_SIMPLE) THEN
      ASM_REWRITE_TAC[NOT_IMP] THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[real_div; REAL_MUL_LZERO] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
        REWRITE_TAC[REAL_ENTIRE] THEN DISJ1_TAC THEN
        CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[BERNOULLI_HALF] THEN
...
let BERNOULLI_NUMBER_EQ_0 = prove
 (`!n. bernoulli n (&0) = &0 <=> ODD n /\ ~(n = 1)`,
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN DISCH_TAC THEN
     ...
     [REWRITE_TAC[IN_REAL_INTERVAL] THEN REPEAT STRIP_TAC THEN
      ...
      ASM_CASES_TAC `m = 0` THENL
       [ASM_REWRITE_TAC[MULT_CLAUSES; ADD_CLAUSES] THEN
        CONV_TAC "(ONCE_DEPTH_CONV BERNOULLI_CONV)" (ONCE_DEPTH_CONV BERNOULLI_CONV) THEN ASM_REAL_ARITH_TAC;
...
let REAL_EULER_MACLAURIN = prove
 (`!f m n p.
  ...
  CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[CONJUNCT1 bernoulli] THEN
  ...
  SPEC_TAC(`p:num`,`p:num`) THEN INDUCT_TAC THENL
   [REWRITE_TAC[SUM_CLAUSES_NUMSEG] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    REAL_ARITH_TAC;
    GEN_REWRITE_TAC (LAND_CONV o ONCE_DEPTH_CONV)
...
let REAL_EULER_MACLAURIN_ANTIDERIVATIVE = prove
 (`!f m n p.
  ...
  CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN DISCH_THEN(SUBST1_TAC o CONJUNCT2) THEN
  MP_TAC(ISPECL
   [`f 0:real->real`; `f (0 + 1):real->real`; `&m`; `&n`]
        REAL_FUNDAMENTAL_THEOREM_OF_CALCULUS) THEN
  ASM_SIMP_TAC[REAL_OF_NUM_LE; LE_0] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  DISCH_THEN(SUBST1_TAC o MATCH_MP REAL_INTEGRAL_UNIQUE) THEN
  AP_TERM_TAC THEN AP_TERM_TAC THEN
 ...
 (`!f:real^N->real^2.
  ...
  MP_TAC THENL
   [X_GEN_TAC `n:num` THEN MATCH_MP_TAC TIETZE_UNBOUNDED THEN CONJ_TAC THENL
    ...
    REWRITE_TAC[VECTOR_SUB_EQ] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN
...
let REAL_MEASURABLE_ON_PREIMAGE_OPEN = prove
 (`!f. f real_measurable_on (:real) <=>
  ...
  MATCH_MP_TAC EQ_IMP THEN AP_TERM_TAC THENL
   [CONV_TAC "SYM_CONV" SYM_CONV; ALL_TAC] THEN
  MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
...
let REAL_LEBESGUE_DIFFERENTIATION_THEOREM_ALT = prove
 (`!f s. is_realinterval s /\ f has_bounded_real_variation_on s
  ...
  REWRITE_TAC[IN_DIFF; IN_ELIM_THM] THEN CONV_TAC "TAUT" TAUT);;
