...
let EULER_MASCHERONI = prove
 (`((\n. sum(1..n) (\k. inv(&k)) - log(&n)) ---> euler_mascheroni)
   sequentially`,
  REWRITE_TAC[euler_mascheroni; reallim] THEN (CONV_TAC "SELECT_CONV") SELECT_CONV THEN
  SUBGOAL_THEN
  ...
  REWRITE_TAC[VSUM_CLAUSES_NUMSEG] THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
...
let LOG2_APPROX_40 = prove
 (`abs(log(&2) - &381061692393 / &549755813888) <= inv(&2 pow 40)`,
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV EXPAND_SUM_CONV)") (ONCE_DEPTH_CONV EXPAND_SUM_CONV) THEN
  (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
 ...
 (`abs(euler_mascheroni - &2479122403 / &4294967296) <= inv(&2 pow 32)`,
  let lemma1 = prove
   (`!m n. 1 <= m /\ m <= n
    ...
    ASM_REWRITE_TAC[] THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN ANTS_TAC THENL
     [REWRITE_TAC[IN_REAL_INTERVAL] THEN REPEAT
      ...
      (CONV_TAC "(ONCE_DEPTH_CONV BERNOULLI_CONV)") (ONCE_DEPTH_CONV BERNOULLI_CONV) THEN
    ...
    ASM_REWRITE_TAC[REAL_OF_NUM_LE] THEN ANTS_TAC THENL
     [REWRITE_TAC[IN_REAL_INTERVAL] THEN REPEAT STRIP_TAC THEN
      REAL_DIFF_TAC THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
    ...
    (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_MUL_LID] THEN
  ...
  REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [GEN_TAC THEN
    ...
    REWRITE_TAC[REALLIM_1_OVER_N] THEN
    MATCH_MP_TAC REALLIM_1_OVER_POW THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV;
    ALL_TAC] THEN
  ...
  SUBGOAL_THEN `log(&64) = &6 * log(&2)` SUBST1_TAC THENL
   [SIMP_TAC[GSYM LOG_POW; REAL_ARITH `&0 < &2`] THEN
    AP_TERM_TAC THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV;
    (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
    (CONV_TAC "(ONCE_DEPTH_CONV EXPAND_SUM_CONV)") (ONCE_DEPTH_CONV EXPAND_SUM_CONV) THEN
    (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
...
let LGAMMA,COMPLEX_DIFFERENTIABLE_AT_LGAMMA = (CONJ_PAIR o prove)
 (`(!z. (!n. ~(z + Cx(&n) = Cx(&0)))
   ...
   [REPEAT STRIP_TAC THEN
    ...
    SUBST1_TAC THENL
     [ASM_SIMP_TAC[GSYM CX_LOG; REAL_OF_NUM_LE; REAL_SUB_LT; REAL_INV_LT_1;
      ...
      REWRITE_TAC[GSYM REAL_OF_NUM_LE] THEN (CONV_TAC "REAL_FIELD") REAL_FIELD;
      ALL_TAC] THEN
  ...
  MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
   [REPEAT STRIP_TAC THEN
    REWRITE_TAC[lgamma; lim] THEN (CONV_TAC "SELECT_CONV") SELECT_CONV THEN
 ...
 (`(!z. ((\n. (Cx(&n) cpow z * Cx(&(FACT n))) / cproduct(0..n) (\m. z + Cx(&m)))
  ...
  ASSUME_TAC THENL
   [MATCH_MP_TAC LIM_TRANSFORM_EVENTUALLY THEN
    ...
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
     [REWRITE_TAC[cgamma; lim] THEN (CONV_TAC "SELECT_CONV") SELECT_CONV THEN ASM_MESON_TAC[];
      DISCH_TAC] THEN
 ...
 (`!z. cgamma(z) = cgamma(z + Cx(&1)) / z`,
  GEN_TAC THEN ASM_CASES_TAC `?n.  z + Cx(&n) = Cx(&0)` THENL
   [FIRST_ASSUM(fun th -> REWRITE_TAC[REWRITE_RULE[GSYM CGAMMA_EQ_0] th]) THEN
    (CONV_TAC "SYM_CONV") SYM_CONV THEN REWRITE_TAC[COMPLEX_DIV_EQ_0] THEN
 ...
 (`!n. ((\z. (z + Cx(&n)) * cgamma z) --> --Cx(&1) pow n / Cx(&(FACT n)))
  ...
  INDUCT_TAC THENL
   [REWRITE_TAC[COMPLEX_ADD_RID; COMPLEX_NEG_0; FACT; complex_pow;
    ...
    CONJ_TAC THENL [(CONV_TAC "COMPLEX_FIELD") COMPLEX_FIELD; ALL_TAC] THEN
    ...
    CONJ_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o
      ...
      CONJ_TAC THENL
       [LIM_TAC THEN (CONV_TAC "COMPLEX_RING") COMPLEX_RING;
 ...
 (`!z. cgamma(z) * cgamma(z + Cx(&1) / Cx(&2)) =
  ...
  CHOOSE_TAC THENL
   [EXISTS_TAC
    ...
    REWRITE_TAC[ADD1] THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
  ...
  ASM_CASES_TAC `!n. ~(Cx(&2) * z + Cx(&n) = Cx(&0))` THENL
   [FIRST_X_ASSUM(fun th ->
    ...
    ANTS_TAC THENL [REAL_ARITH_TAC; (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV] THEN
 ...
 (`!z. cgamma(z) * cgamma(Cx(&1) - z) = Cx pi / csin(Cx pi * z)`,
     ...
     [X_GEN_TAC `w:complex` THEN REWRITE_TAC[COMPLEX_IN_BALL_0] THEN
      STRIP_TAC THEN COND_CASES_TAC THENL
       [UNDISCH_TAC `~(w = Cx(&0))` THEN
        ...
        REWRITE_TAC[GSYM CX_INJ] THEN (CONV_TAC "COMPLEX_FIELD") COMPLEX_FIELD];
  ...
  ASSUME_TAC THENL
   [MATCH_MP_TAC(SET_RULE
    ...
    (CONV_TAC "(DEPTH_BINOP_CONV `==>` (BINOP_CONV(DEPTH_BINOP_CONV `complex_mul` (RAND_CONV COMPLEX_POLY_CONV))))") (DEPTH_BINOP_CONV `==>` (BINOP_CONV(DEPTH_BINOP_CONV `complex_mul` (RAND_CONV COMPLEX_POLY_CONV)))) THEN
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `z:complex` THEN MATCH_MP_TAC
    ...
    (CONJ_TAC THENL [ASM_REWRITE_TAC[]; (CONV_TAC "COMPLEX_RING") COMPLEX_RING]);
    ALL_TAC] THEN
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `z:complex` THEN
    ...
    EXISTS_TAC `z:complex` THEN CONJ_TAC THENL
     [REWRITE_TAC[]; ASM_REWRITE_TAC[]] THEN
    ASM GEN_COMPLEX_DIFF_TAC [] THEN
    (CONJ_TAC THENL [ASM_REWRITE_TAC[ETA_AX]; (CONV_TAC "COMPLEX_RING") COMPLEX_RING]);
    ALL_TAC] THEN
  MP_TAC(ISPECL [`h':complex->complex`; `h'':complex->complex`; `(:complex)`]
        HOLOMORPHIC_DERIVATIVE) THEN
  ASM_REWRITE_TAC[OPEN_UNIV] THEN DISCH_TAC THEN
  SUBGOAL_THEN `!z. z IN (:complex) ==> h''(z) = Cx(&0)` MP_TAC THENL
   [MATCH_MP_TAC ANALYTIC_CONTINUATION THEN
    ...
    REWRITE_TAC[o_DEF] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN ANTS_TAC THENL
     [MATCH_MP_TAC CONTINUOUS_ON_LIFT_NORM_COMPOSE THEN
      ...
      MP_TAC(SPEC `&1` COMPLEX_NORM_CX) THEN (CONV_TAC "NORM_ARITH") NORM_ARITH];
  ...
  REWRITE_TAC[CONVEX_UNIV; IN_UNIV] THEN ANTS_TAC THENL
   [GEN_TAC THEN ASM GEN_COMPLEX_DIFF_TAC[] THEN
    ASM_REWRITE_TAC[] THEN (CONV_TAC "COMPLEX_RING") COMPLEX_RING;
   ...
   [SUBST1_TAC(SYM(SPEC `a:complex` COMPLEX)) THEN
    ...
    ASM_CASES_TAC `?n. integer n /\ z = Cx n` THENL
     [REWRITE_TAC[complex_div] THEN
      ...
      MP_TAC PI_NZ THEN REWRITE_TAC[GSYM CX_INJ] THEN (CONV_TAC "COMPLEX_FIELD") COMPLEX_FIELD];
    DISCH_THEN(fun th ->
...
let CGAMMA_HALF = prove
 (`cgamma(Cx(&1) / Cx(&2)) = Cx(sqrt pi)`,
  ...
  SUBGOAL_THEN `Cx pi = Cx(sqrt pi) pow 2` SUBST1_TAC THENL
   [REWRITE_TAC[GSYM CX_POW] THEN AP_TERM_TAC THEN (CONV_TAC "SYM_CONV") SYM_CONV THEN
    ...
    (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN MATCH_MP_TAC(TAUT `~p ==> p ==> q`) THEN
 ...
 (`!z. ((\n. cexp(--(Cx euler_mascheroni) * z) / z *
  ...
  REWRITE_TAC[EVENTUALLY_SEQUENTIALLY] THEN CONJ_TAC THENL
   [EXISTS_TAC `1` THEN REPEAT STRIP_TAC THEN REWRITE_TAC[complex_div] THEN
    ...
    ASM_SIMP_TAC[CX_LOG; REAL_OF_NUM_LT; LE_1] THEN (CONV_TAC "COMPLEX_RING") COMPLEX_RING;
    ALL_TAC] THEN
  GEN_REWRITE_TAC LAND_CONV [GSYM COMPLEX_MUL_LID] THEN
  MATCH_MP_TAC LIM_COMPLEX_MUL THEN CONJ_TAC THENL
   [SIMP_TAC[GSYM CEXP_VSUM; FINITE_NUMSEG; GSYM CEXP_ADD] THEN
    ...
    (CONV_TAC "SYM_CONV") SYM_CONV THEN SPEC_TAC(`n:num`,`p:num`) THEN
    INDUCT_TAC THEN REWRITE_TAC[FACT; CPRODUCT_CLAUSES_NUMSEG; ARITH] THEN
 ...
 (`!z. (inv o cgamma) complex_differentiable (at z)`,
  GEN_TAC THEN ASM_CASES_TAC `!n. ~(z + Cx(&n) = Cx(&0))` THENL
   [MATCH_MP_TAC COMPLEX_DIFFERENTIABLE_COMPOSE_AT THEN
    ...
    ONCE_REWRITE_TAC[COMPLEX_MUL_SYM] THEN
    REWRITE_TAC[CGAMMA_SIMPLE_POLES] THEN (CONV_TAC "COMPLEX_RING") COMPLEX_RING]);;
   ...
   [REPEAT STRIP_TAC THEN
    SUBGOAL_THEN `(f:real->real) n = f((n - &1) + &1)` SUBST1_TAC THENL
     [AP_TERM_TAC THEN REAL_ARITH_TAC;
      ...
      UNDISCH_TAC `&2 <= n` THEN (CONV_TAC "REAL_FIELD") REAL_FIELD];
     ...
     [REWRITE_TAC[complex_pow; COMPLEX_MUL_RID; CPRODUCT_CLAUSES_NUMSEG] THEN
      ...
      DISCH_THEN MATCH_MP_TAC THEN REWRITE_TAC[REAL_POS] THEN CONJ_TAC THENL
       [MATCH_MP_TAC CONTINUOUS_ON_COMPLEX_RMUL THEN
        ...
        UNDISCH_TAC `~(z = Cx(&0))` THEN (CONV_TAC "COMPLEX_FIELD") COMPLEX_FIELD];
 ...
 (`(!z n. 1 <= n /\ (Im z = &0 ==> &0 < Re z)
  ...
  let lemma1 = prove
   (`!p n z. (Im z = &0 ==> &0 < Re z)
    ...
    (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN ANTS_TAC THENL
      ...
      ASM_CASES_TAC `k = 0` THEN ASM_REWRITE_TAC[] THENL
       [COMPLEX_DIFF_TAC THEN CONJ_TAC THENL
         [REWRITE_TAC[RE_ADD; IM_ADD; RE_CX; IM_CX] THEN ASM_REAL_ARITH_TAC;
          UNDISCH_TAC `~(z + Cx x = Cx(&0))` THEN (CONV_TAC "COMPLEX_FIELD") COMPLEX_FIELD];
        ALL_TAC] THEN
    ...
    (CONV_TAC "COMPLEX_RING") COMPLEX_RING) in
  let lemma2 = prove
     ...
     [REWRITE_TAC[GSYM VSUM_NEG] THEN MATCH_MP_TAC LIM_VSUM THEN
      ...
      THENL
       [MATCH_MP_TAC(COMPLEX_FIELD
        ...
        (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN DISCH_THEN (K ALL_TAC) THEN
     ...
     [REWRITE_TAC[CLOG_1; COMPLEX_MUL_RZERO; COMPLEX_SUB_RZERO] THEN
      ...
      MATCH_MP_TAC LIM_TRANSFORM_EVENTUALLY THEN EXISTS_TAC
       `\n. z * (clog(Cx(&n)) - clog(z + Cx(&n))) +
                ...
                (Cx(&1) - z)` THEN
      CONJ_TAC THENL
       [MATCH_MP_TAC ALWAYS_EVENTUALLY THEN (CONV_TAC "COMPLEX_RING") COMPLEX_RING;
        ALL_TAC] THEN
  ...
  let lemma7 = prove
   (`((\y. integral {t | &0 <= drop t}
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LT_MUL THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
  ...
  let lemma8 = prove
   (`integral {t | &0 <= drop t}
    ...
    CONJ_TAC THENL [ALL_TAC; (CONV_TAC "COMPLEX_RING") COMPLEX_RING] THEN
    ...
    REWRITE_TAC[CLOG_1] THEN ANTS_TAC THENL
     [SIMP_TAC[CONTINUOUS_AT_CLOG; RE_CX; REAL_LT_01; CX_SUB] THEN
      ...
      MATCH_MP_TAC LIM_NULL_COMPLEX_POW THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
...
let WALLIS_ALT = prove
 (`((\n. product(1..n) (\k. (&2 * &k) / (&2 * &k - &1) *
  ...
  CONJ_TAC THENL [ALL_TAC; MP_TAC PI_POS THEN (CONV_TAC "REAL_FIELD") REAL_FIELD] THEN
...
let WALLIS = prove
 (`((\n. (&2 pow n * &(FACT n)) pow 4 / (&(FACT(2 * n)) * &(FACT(2 * n + 1))))
  ...
  REWRITE_TAC[GSYM REAL_OF_NUM_EQ] THEN (CONV_TAC "REAL_FIELD") REAL_FIELD);;
 ...
 (`((\x. exp(--(x pow 2))) has_real_integral sqrt pi) (:real)`,
  ...
  MP_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    REWRITE_TAC[] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
    ...
    ANTS_TAC THENL
     [REPEAT CONJ_TAC THENL
      ...
      ASM_SIMP_TAC[RPOW_RPOW] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
      REWRITE_TAC[RPOW_NEG; RPOW_POW; REAL_POW_1] THEN
      MP_TAC(SPEC `(x:real) pow 2` REAL_EXP_NZ) THEN
      UNDISCH_TAC `~(x = &0)` THEN (CONV_TAC "REAL_FIELD") REAL_FIELD];
  ...
  SIMP_TAC[] THEN DISCH_THEN(K ALL_TAC) THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `e:real`) THEN ASM_REWRITE_TAC[] THEN
  DISCH_THEN(X_CHOOSE_THEN `B:real` STRIP_ASSUME_TAC) THEN
