...
let DIST_LE_CASES,DIST_ADDBOUND,DIST_TRIANGLE,DIST_ADD2,DIST_ADD2_REV =
  let DIST_ELIM_TAC =
    ...
    CONV_TAC "conv" conv THEN TRY GEN_TAC THEN CONJ_TAC THEN
  ...
  let DIST_ELIM_TAC' =
    ...
    CONV_TAC "(LAND_CONV NUM_CANCEL_CONV)" (LAND_CONV NUM_CANCEL_CONV) THEN
 ...
 (`!x. ?B. !m n. dist(fn x (m * n),m * fn x n) <= B * m + B`,
  ...
  ASM_CASES_TAC `n = 0` THENL
   [MATCH_MP_TAC (LE_IMP DIST_ADDBOUND) THEN
    ...
    REWRITE_TAC[LE_EXISTS] THEN CONV_TAC "(ONCE_DEPTH_CONV NUM_CANCEL_CONV)" (ONCE_DEPTH_CONV NUM_CANCEL_CONV) THEN
  ...
  CONV_TAC "(ONCE_DEPTH_CONV NUM_CANCEL_CONV)" (ONCE_DEPTH_CONV NUM_CANCEL_CONV) THEN
 ...
 (`!x. ?B. !m n. dist(fn x (m + n),fn x m + fn x n) <= B`,
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[SYM(REWRITE_CONV [ARITH] `1 + 1 + 1`)] THEN
    ...
    REWRITE_TAC[MULT_AC] THEN CONV_TAC "NUM_CANCEL_CONV" NUM_CANCEL_CONV THEN REFL_TAC;
...
let NADD_LE_TOTAL_LEMMA = prove
 (`!x y. ~(x <<= y) ==> !B. ?n. ~(n = 0) /\ fn y n + B < fn x n`,
  ...
  CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[NOT_LT] THEN
...
let NADD_ARCH_ZERO = prove
 (`!x k. (!n. &n ** x <<= k) ==> (x === &0)`,
  REPEAT GEN_TAC THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN DISCH_TAC THEN
