...
let num_cases' = prove
 (`!t z. ~(z IN FVT t)
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV TERMSUBST_CONV)") (ONCE_DEPTH_CONV TERMSUBST_CONV) THEN
 ...
 (`!t. {robinson} |-- Suc t === Z --> False`,
  ...
  SUBGOAL_THEN `{robinson} |-- ?? 0 (Suc(V 1) === Suc(V 0))` MP_TAC THENL
   [MATCH_MP_TAC exists_intro THEN EXISTS_TAC `V 1` THEN
    (CONV_TAC "(RAND_CONV FORMSUBST_CONV)") (RAND_CONV FORMSUBST_CONV) THEN REWRITE_TAC[axiom_eqrefl];
 ...
 (`!m n. m <= n ==> {robinson} |-- numeral m <<= numeral n`,
  ...
  CONJ_TAC THENL
   [MP_TAC(itlist spec_rule [`numeral(m + n)`; `numeral m`] le_def) THEN
    ...
    (CONV_TAC "(RAND_CONV FORMSUBST_CONV)") (RAND_CONV FORMSUBST_CONV) THEN
 ...
 (`!p x. {robinson} |-- formsubst (x |=> Z) p /\
  ...
  let lemma = prove
   (`!A p x t. A |-- formsubst (x |=> t) p ==> A |-- V x === t --> p`,
    ...
    (CONV_TAC "(funpow 3 RAND_CONV(SUBS_CONV[SYM(SPEC `x:num` ASSIGN_TRIV)]))") (funpow 3 RAND_CONV(SUBS_CONV[SYM(SPEC `x:num` ASSIGN_TRIV)])) THEN
  ...
  (CONV_TAC "(RAND_CONV(SUBS_CONV[SYM(SPEC `x:num` ASSIGN_TRIV)]))") (RAND_CONV(SUBS_CONV[SYM(SPEC `x:num` ASSIGN_TRIV)])) THEN
