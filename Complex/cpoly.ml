   ...
   [INDUCT_TAC THENL
    ...
    REWRITE_TAC[divides] THEN CONJ_TAC THENL
     [EXISTS_TAC `s:complex list` THEN
      ...
      SPEC_TAC(`n:num`,`n:num`) THEN INDUCT_TAC THEN
      ASM_REWRITE_TAC[] THENL
       [UNDISCH_TAC `~(poly s a = Cx(&0))` THEN CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN
  ...
  EQ_TAC THEN STRIP_TAC THENL
   [SUBGOAL_THEN `~(poly p = poly [])` ASSUME_TAC THENL
     [FIRST_ASSUM(UNDISCH_TAC o check is_neg o concl) THEN
      CONV_TAC "CONTRAPOS_CONV" CONTRAPOS_CONV THEN REWRITE_TAC[divides] THEN
    ...
    ONCE_ASM_REWRITE_TAC[] THEN CONV_TAC "SELECT_CONV" SELECT_CONV] THEN
