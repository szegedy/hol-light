 ...
 (`!n. (&3 / (&16 * ln(&32))) * sum(1..n) (\i. &1 / &i) <=
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[ARITH; SUM_SING_NUMSEG] THEN
    CONV_TAC "(RAND_CONV SUM_CONV)" (RAND_CONV SUM_CONV) THEN REWRITE_TAC[] THEN
    CONV_TAC "(ONCE_DEPTH_CONV PRIME_CONV)" (ONCE_DEPTH_CONV PRIME_CONV) THEN
    ...
    ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
