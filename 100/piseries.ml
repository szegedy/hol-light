       ...
       [EXISTS_TAC `x:num` THEN EXPAND_TAC "q" THEN BETA_TAC THEN
        ...
        DISCH_THEN(DISJ_CASES_THEN2 ASSUME_TAC SUBST_ALL_TAC) THENL
         [EXISTS_TAC `x - 1` THEN EXPAND_TAC "q" THEN BETA_TAC THEN
          ...
          REWRITE_TAC[GSYM LE_SUC_LT] THEN (CONV_TAC "CONTRAPOS_CONV") CONTRAPOS_CONV THEN
...
let COT_HALF_MULTIPLE = prove
 (`~(integer x)
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV (ALPHA_CONV `j:num`))") (ONCE_DEPTH_CONV (ALPHA_CONV `j:num`)) THEN
 ...
 (`~(integer x)
  ...
  BINOP_TAC THENL
   [GEN_REWRITE_TAC (LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
    ...
    (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_MUL_RID]; ALL_TAC] THEN
...
let NOT_INTEGER_LEMMA = prove
 (`~(x = &0) /\ abs(x) < &1 ==> ~(integer x)`,
  ...
  (CONV_TAC "CONTRAPOS_CONV") CONTRAPOS_CONV THEN SIMP_TAC[integer; LEFT_IMP_EXISTS_THM] THEN
...
let SIN_LINEAR_ABOVE = prove
 (`!x. abs(x) < &1 ==> abs(sin x) <= &2 * abs(x)`,
  ...
  CONJ_TAC THENL [ALL_TAC; (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV] THEN
...
let SIN_LINEAR_BELOW = prove
 (`!x. abs(x) < &2 ==> abs(sin x) >= abs(x) / &3`,
  ...
  (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
  (CONV_TAC "(LAND_CONV(RAND_CONV(RAND_CONV num_CONV)))") (LAND_CONV(RAND_CONV(RAND_CONV num_CONV))) THEN
  ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
 ...
 (`(\n. inv(&(n + 1) * &(n + 2))) sums &1`,
  ...
  THENL
   [INDUCT_TAC THEN REWRITE_TAC[sum] THEN
    ...
    (CONV_TAC "SYM_CONV") SYM_CONV THEN MATCH_MP_TAC REAL_MUL_LINV_UNIQ THEN
    ONCE_REWRITE_TAC[REAL_MUL_SYM] THEN
 ...
 (`((\x. x * cot(x)) tends_real_real &1)(&0)`,
  SUBGOAL_THEN `(cos tends_real_real &1)(&0)` MP_TAC THENL
   [MP_TAC(SPEC `&0` DIFF_COS) THEN
    ...
    (CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)") (ONCE_DEPTH_CONV ETA_CONV) THEN REWRITE_TAC[]; ALL_TAC] THEN
   ...
   [X_GEN_TAC `x:real` THEN STRIP_TAC THEN
    ...
    REWRITE_TAC[cot] THEN MATCH_MP_TAC CONT_DIV THEN REPEAT CONJ_TAC THENL
     [MATCH_MP_TAC DIFF_CONT THEN
      ...
      (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV]; ALL_TAC] THEN
   ...
   [X_GEN_TAC `e:real` THEN DISCH_TAC THEN UNDISCH_TAC `z sums l` THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC REAL_LE_TRANS THEN
      ...
      (CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)") (ONCE_DEPTH_CONV ETA_CONV) THEN ASM_REWRITE_TAC[]; ALL_TAC] THEN
    MATCH_MP_TAC REAL_LTE_TRANS THEN EXISTS_TAC `e / &2 + e / &2` THEN
    CONJ_TAC THENL
     [ALL_TAC;
      SIMP_TAC[REAL_LE_REFL; GSYM REAL_MUL_2; REAL_DIV_LMUL;
    ...
    EXISTS_TAC `sum(0,M) (\n. e / (&2 * &(M + 1)))` THEN CONJ_TAC THENL
     [MATCH_MP_TAC SUM_LE THEN (CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)") (ONCE_DEPTH_CONV ETA_CONV) THEN
...
let TAN_DERIV_POWSER = prove
 (`!n x. abs(x) < pi / &2
  ...
  REWRITE_TAC[ITER] THEN (CONV_TAC "(ONCE_DEPTH_CONV ETA_CONV)") (ONCE_DEPTH_CONV ETA_CONV) THEN
...
let HARMONICSUMS_TANNUMBER = prove
 (`!n. EVEN n /\ ~(n = 0)
  ...
  REWRITE_TAC[GSYM real_div] THEN (CONV_TAC "SYM_CONV") SYM_CONV THEN
 ...
 (`!n x. (poly (tanpoly n) (--x) =
  ...
  INDUCT_TAC THENL
   [REWRITE_TAC[EVEN; tanpoly] THEN
    (CONV_TAC "(ONCE_DEPTH_CONV POLY_DIFF_CONV)") (ONCE_DEPTH_CONV POLY_DIFF_CONV) THEN
   ...
   [REWRITE_TAC[REAL_ABS_MUL; REAL_ABS_NUM] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC(REAL_ARITH `(y = x) ==> x <= y`) THEN
      ...
      MATCH_MP_TAC SUMMABLE_INVERSE_POWERS THEN REWRITE_TAC[LE_REFL];
      ALL_TAC] THEN
    REWRITE_TAC[SUM_1; ADD_CLAUSES] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
 ...
 (`!x n. abs(x) <= &1
  ...
  SUBGOAL_THEN `~(&2 / pi * abs(x) = &1)` ASSUME_TAC THENL
   [UNDISCH_TAC `&2 / pi * abs x < &2 / &3` THEN
    ...
    SIMP_TAC[] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
  ...
  REWRITE_TAC[GSYM real_div] THEN CONJ_TAC THENL
   [MATCH_MP_TAC(REAL_ARITH
    ...
    ((CONV_TAC "o") o GEN_SIMPLIFY_CONV TOP_DEPTH_SQCONV (basic_ss []) 4)
...
let TAYLOR_TANX_BOUND = prove
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(x = &0)
  ...
  REWRITE_TAC[SUM_1] THEN
  (CONV_TAC "(ONCE_DEPTH_CONV TANNUMBER_CONV)") (ONCE_DEPTH_CONV TANNUMBER_CONV) THEN
  (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN REWRITE_TAC[real_pow] THEN
  (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_SUB_RZERO] THEN
  MATCH_MP_TAC REAL_LE_TRANS THEN
 ...
 (`!x n. abs(x) <= &1 /\ ~(x = &0)
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_LE THEN
    ...
    REWRITE_TAC[REAL_MUL_ASSOC] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
...
let TAYLOR_COTXX_BOUND = prove
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(x = &0)
  ...
  ASM_CASES_TAC `n = 0` THENL
   [ASM_REWRITE_TAC[] THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN REWRITE_TAC[sum] THEN
  ...
  (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN
  MATCH_MP_TAC REAL_LE_LMUL THEN REWRITE_TAC[REAL_POS] THEN
  REWRITE_TAC[real_div; REAL_MUL_LID] THEN
  GEN_REWRITE_TAC (LAND_CONV o LAND_CONV) [REAL_MUL_SYM] THEN
  REWRITE_TAC[REAL_POW_MUL; REAL_POW_INV] THEN
