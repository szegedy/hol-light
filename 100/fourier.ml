...
let SQUARE_INTEGRABLE_0 = prove
 (`!s. (\x. &0) square_integrable_on s`,
  ...
  (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_INTEGRABLE_0]);;
...
let L2NORM_LNORM = prove
 (`!f s. f square_integrable_on s
  ...
  REWRITE_TAC[REAL_POW2_ABS] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
...
let L2_SUMMABLE = prove
 (`!f s t.
  ...
  (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN ANTS_TAC THENL
...
let RIEMANN_LEBESGUE_SIN_HALF = prove
 (`!f. f absolutely_real_integrable_on real_interval[--pi,pi]
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC ALWAYS_EVENTUALLY THEN X_GEN_TAC `n:num` THEN
    REWRITE_TAC[GSYM REAL_MUL_ASSOC] THEN (CONV_TAC "SYM_CONV") SYM_CONV THEN
...
let FOURIER_SUM_0 = prove
 (`!f n.
  ...
  CONJ_TAC THENL
   [(CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN (CONV_TAC "SYM_CONV") SYM_CONV THEN
...
let FOURIER_SUM_LIMIT_DIRICHLET_KERNEL = prove
 (`!f t l.
  ...
  THENL [MP_TAC PI_POS THEN (CONV_TAC "REAL_FIELD") REAL_FIELD; ALL_TAC] THEN
 ...
 (`!f d M a t.
  ...
  STRIP_ASSUME_TAC THENL
   [MP_TAC(REAL_DIFF_CONV
    ...
    (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[COS_0; REAL_MUL_RID] THEN
    REWRITE_TAC[HAS_REAL_DERIVATIVE_ATREAL; REALLIM_ATREAL] THEN
    DISCH_THEN(MP_TAC o SPEC `&1 / &4`) THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
    ...
    REWRITE_TAC[PI_POS_LE] THEN ANTS_TAC THENL
     [CONJ_TAC THENL
       [MATCH_MP_TAC REAL_CONTINUOUS_ON_LMUL THEN
        ...
        REAL_DIFF_TAC THEN
        MAP_EVERY UNDISCH_TAC [`&0 < a`; `&0 < x`] THEN (CONV_TAC "REAL_FIELD") REAL_FIELD];
...
let FOURIER_SUM_LIMIT_DIRICHLET_KERNEL_HALF = prove
 (`!f t l.
  ...
  MP_TAC PI_POS THEN (CONV_TAC "REAL_FIELD") REAL_FIELD);;
