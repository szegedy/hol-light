...
let WILSON_LEMMA = prove
 (`!p. prime(p) ==> (FACT(p - 2) == 1) (mod p)`,
  ...
  THENL [(CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN (CONV_TAC "CONG_CONV") CONG_CONV; ALL_TAC] THEN
...
let WILSON = prove
 (`!p. ~(p = 1) ==> (prime p <=> (FACT(p - 1) == p - 1) (mod p))`,
  ...
  ASM_CASES_TAC `n = 0` THEN ASM_REWRITE_TAC[] THENL
   [ASM_REWRITE_TAC[CONG_MOD_0] THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV; ALL_TAC] THEN
...
let EULER_CRITERION = prove
 (`!a p. prime p /\ coprime(a,p)
  ...
  REPEAT STRIP_TAC THEN FIRST_ASSUM(DISJ_CASES_THEN2 SUBST_ALL_TAC ASSUME_TAC o
    MATCH_MP PRIME_ODD) THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
...
let GAUSS_LEMMA_SYM = prove
 (`!p q r s. prime p /\ prime q /\ coprime(p,q) /\
  ...
  CONJ_TAC THENL
   [(CONV_TAC "SYM_CONV") SYM_CONV THEN MATCH_MP_TAC CARD_SUBCROSS_DETERMINATE THEN
