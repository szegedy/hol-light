 ...
 (`!n a b.
  ...
  THENL
   [FIRST_X_ASSUM SUBST_ALL_TAC THEN EXISTS_TAC `1` THEN
    ...
    CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN COND_CASES_TAC THEN REWRITE_TAC[] THEN
    ASM_ARITH_TAC;
    ALL_TAC] THEN
   ...
   [X_GEN_TAC `m:num` THEN MATCH_MP_TAC EQ_TRANS THEN
    EXISTS_TAC `sum(0..m+1) a` THEN CONJ_TAC THENL
     [SIMP_TAC[SUM_CLAUSES_LEFT; LE_0; ARITH_RULE `0 + 1 <= n + 1`] THEN
      CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN REWRITE_TAC[REAL_ADD_ASSOC] THEN
     ...
     [MATCH_MP_TAC EQ_TRANS THEN
      EXISTS_TAC `variation(0..p) b + variation(p..n) b` THEN CONJ_TAC THENL
       [MATCH_MP_TAC VARIATION_SPLIT_NUMSEG THEN REWRITE_TAC[IN_NUMSEG] THEN
        ...
        FIRST_X_ASSUM MATCH_MP_TAC THEN ASM_ARITH_TAC;
        BINOP_TAC THENL [ALL_TAC; CONV_TAC "SYM_CONV" SYM_CONV] THEN
     ...
     [MATCH_MP_TAC EQ_TRANS THEN
      EXISTS_TAC `variation(0..1) b + variation(1..n) b` THEN CONJ_TAC THENL
       [MATCH_MP_TAC VARIATION_SPLIT_NUMSEG THEN REWRITE_TAC[IN_NUMSEG] THEN
        ...
        CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_REWRITE_TAC[];
      ...
      SUBGOAL_THEN `(b:num->real) 1 = a 0 + a 1` ASSUME_TAC THENL
       [FIRST_ASSUM(fun th -> GEN_REWRITE_TAC LAND_CONV [GSYM th]) THEN
        ...
        ALL_TAC]]] THEN
  REPEAT(FIRST_X_ASSUM(MP_TAC o SPEC `0`)) THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ...
  EXPAND_TAC "a'" THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  ASM_SIMP_TAC[ARITH_RULE `1 < p ==> ~(p = 1)`; REAL_LT_REFL] THEN
  REPEAT(COND_CASES_TAC THEN ASM_REWRITE_TAC[]) THEN
  CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  CONV_TAC "(BINDER_CONV(RAND_CONV(RAND_CONV INT_POLY_CONV)))" (BINDER_CONV(RAND_CONV(RAND_CONV INT_POLY_CONV))) THEN
...
let MULTIPLICITY_UNIQUE = prove
 (`!f a r b m k.
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SELECT_UNIQUE THEN
...
let MULTIPLICITY_WORKS = prove
 (`!r n a. 
  ...
  REWRITE_TAC[multiplicity] THEN CONV_TAC "(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
