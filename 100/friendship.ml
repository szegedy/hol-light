...
let EQUIVALENCE_UNIFORM_PARTITION = prove
 (`!R s k. FINITE s /\
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_UNION_EQ THEN ASM SET_TAC[]);;
 ...
 (`!friend:person->person->bool.
  ...
  SUBGOAL_THEN `~(m = 2)` ASSUME_TAC THENL
   [DISCH_THEN SUBST_ALL_TAC THEN
    ...
    CONV_TAC "(LAND_CONV HAS_SIZE_CONV)" (LAND_CONV HAS_SIZE_CONV) THEN REWRITE_TAC[NOT_EXISTS_THM] THEN
