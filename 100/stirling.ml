...
let STIRLING_UPPERBOUND = prove
 (`!n. stirling(SUC n) <= &1`,
  INDUCT_TAC THENL
   [REWRITE_TAC[stirling] THEN (CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN
    REWRITE_TAC[LN_1] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
 ...
 (`!n. (&n + &2) * integral(&0,pi / &2) (\x. sin(x) pow (n + 2)) =
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [SIMP_TAC[REAL_LT_IMP_LE; PI_POS; REAL_LT_DIV; REAL_OF_NUM_LT; ARITH] THEN
    (CONV_TAC "(ONCE_DEPTH_CONV INTEGRABLE_CONV)") (ONCE_DEPTH_CONV INTEGRABLE_CONV) THEN REWRITE_TAC[] THEN
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV INTEGRABLE_CONV)") (ONCE_DEPTH_CONV INTEGRABLE_CONV) THEN SIMP_TAC[PI2_LE]);;
...
let WALLIS_1 = prove
 (`integral(&0,pi / &2) (\x. sin(x) pow 1) = &1`,
  ...
  REWRITE_TAC[COS_0; COS_PI2] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
...
let WALLIS_ODD = prove
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (2 * n + 1)) =
  ...
  INDUCT_TAC THENL
   [(CONV_TAC "NUM_REDUCE_CONV") NUM_REDUCE_CONV THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
    REWRITE_TAC[WALLIS_1] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV;
    ALL_TAC] THEN
...
let WALLIS_QUOTIENT' = prove
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (2 * n)) /
  ...
  MP_TAC PI2_NZ THEN (CONV_TAC "REAL_FIELD") REAL_FIELD);;
...
let WALLIS_MONO = prove
 (`!m n. m <= n
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV INTEGRABLE_CONV)") (ONCE_DEPTH_CONV INTEGRABLE_CONV) THEN REWRITE_TAC[PI2_LE] THEN
 ...
 (`!n. integral(&0,pi / &2) (\x. sin(x) pow (n + 1))
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[WALLIS_PARTS'] THEN
    MATCH_MP_TAC REAL_EQ_IMP_LE THEN (CONV_TAC "REAL_FIELD") REAL_FIELD;
    ALL_TAC] THEN
...
let WALLIS = prove
 (`(\n. (&2 pow n * &(FACT n)) pow 4 / (&(FACT(2 * n)) * &(FACT(2 * n + 1))))
  ...
  CONJ_TAC THENL [ALL_TAC; MP_TAC PI2_NZ THEN (CONV_TAC "REAL_FIELD") REAL_FIELD] THEN
...
let STIRLING = prove
 (`(\n. ln(&(FACT n)) - ((&n + &1 / &2) * ln(&n) - &n + ln(&2 * pi) / &2))
  ...
  (CONV_TAC "(LAND_CONV(GEN_ALPHA_CONV `n:num`))") (LAND_CONV(GEN_ALPHA_CONV `n:num`)) THEN
  REWRITE_TAC[REAL_ARITH
  ...
  DISCH_THEN MATCH_MP_TAC THEN (CONV_TAC "(LAND_CONV(GEN_ALPHA_CONV `n:num`))") (LAND_CONV(GEN_ALPHA_CONV `n:num`)) THEN
