...
let LIOUVILLE_SUM_BOUND = prove
 (`!d n. ~(n = 0)
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[GSYM ADD1; FACT] THEN
...
let TRANSCENDENTAL_LIOUVILLE = prove
 (`transcendental(liouville)`,
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN DISCH_THEN(X_CHOOSE_TAC `k:num`) THEN
