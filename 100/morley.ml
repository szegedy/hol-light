...
let EQUILATERAL_TRIANGLE_ALGEBRAIC = prove
 (`!A B C j.
  ...
  (CONJUNCTS_THEN SUBST1_TAC) THENL
   [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "COMPLEX_RING" COMPLEX_RING;
    ALL_TAC] THEN
...
let AFFINE_GROUP_ITER_3 = prove
 (`ITER 3 (\z. a * z + b) = (\z. a pow 3 * z + b * (Cx(&1) + a + a pow 2))`,
  ...
  REWRITE_TAC[ITER; FUN_EQ_THM] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  CONV_TAC "COMPLEX_RING" COMPLEX_RING);;

let AFFINE_GROUP_COMPOSE = prove
 (`(\z. a1 * z + b1) o (\z. a2 * z + b2) =
   (\z. (a1 * a2) * z + (b1 + a1 * b2))`,
  REWRITE_TAC[o_THM; FUN_EQ_THM] THEN CONV_TAC "COMPLEX_RING" COMPLEX_RING);;

let AFFINE_GROUP_I = prove
 (`I = (\z. Cx(&1) * z + Cx(&0))`,
  REWRITE_TAC[I_THM; FUN_EQ_THM] THEN CONV_TAC "COMPLEX_RING" COMPLEX_RING);;
...
let ALGEBRAIC_LEMMA = prove
 (`!a1 a2 a3 b1 b2 b3 A B C.
  ...
  REPEAT GEN_TAC THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
   [REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "COMPLEX_RING" COMPLEX_RING; ALL_TAC] THEN
  SUBGOAL_THEN
   ...
   [MAP_EVERY EXPAND_TAC ["g1"; "g3"] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC ROTATE_EQ_REFLECT_LEMMA THEN
      ...
      ALL_TAC] THEN
    CONV_TAC "SYM_CONV" SYM_CONV THEN ONCE_REWRITE_TAC[REFLECT_ACROSS_SYM] THEN
