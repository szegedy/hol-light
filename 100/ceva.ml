 ...
 (`!a b c:real^2.
  ...
  let lemma = prove
   (`~(y1 = &0) /\ x2 * y1 = x1 * y2 ==> ?c. x1 = c * y1 /\ x2 = c * y2`,
    ...
    REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "REAL_FIELD" REAL_FIELD) in
  ...
  MATCH_MP_TAC lemma THEN REPEAT(POP_ASSUM MP_TAC) THEN CONV_TAC "REAL_FIELD" REAL_FIELD);;
 ...
 (`!A B C X Y Z:real^2.
  ...
  EQ_TAC THENL
   [ALL_TAC;
    ...
    CONV_TAC "REAL_RING" REAL_RING] THEN
  ...
  CONJ_TAC THENL
   [GEN_TAC THEN
    ...
    REPEAT(MATCH_MP_TAC(TAUT `(a ==> b /\ c) /\ (a /\ b /\ c ==> d)
                              ==> a ==> b /\ c /\ d`) THEN
           CONJ_TAC THENL
            [CONV_TAC "REAL_RING" REAL_RING ORELSE CONV_TAC "REAL_SOS" REAL_SOS; ALL_TAC]) THEN
    CONV_TAC "REAL_SOS" REAL_SOS;
    ALL_TAC] THEN
  RULE_ASSUM_TAC(REWRITE_RULE[COLLINEAR]) THEN
  ASM_CASES_TAC `x = &0` THENL
   [EXISTS_TAC `&1 - y / (&1 - x + x * y)` THEN
    REPEAT(FIRST_X_ASSUM(MP_TAC o check (not o is_neg o concl))) THEN
    CONV_TAC "REAL_FIELD" REAL_FIELD; ALL_TAC] THEN
...
let BETWEEN_METRICAL = prove
 (`!u v w:real^N. between v (u,w) <=> dist(u,v) + dist(v,w) = dist(u,w)`,
  ...
  CONV_TAC "(LAND_CONV SYM_CONV)" (LAND_CONV SYM_CONV) THEN REWRITE_TAC[NORM_TRIANGLE_EQ] THEN
  EQ_TAC THENL
   [ALL_TAC;
