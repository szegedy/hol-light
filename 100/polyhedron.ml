...
let HYPERPLANE_CELL_CONVEX = prove
 (`!A c:real^N->bool. hyperplane_cell A c ==> convex c`,
  ...
  CONV_TAC "(ONCE_DEPTH_CONV SYM_CONV)" (ONCE_DEPTH_CONV SYM_CONV) THEN REWRITE_TAC[hyperplane_side] THEN
  REPEAT_TCL DISJ_CASES_THEN SUBST1_TAC
 ...
 (`!A h s:real^N->bool.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC(MESON[] `s = t ==> sum s f = sum t f`) THEN
    ...
    REWRITE_TAC[IN_INSERT; NOT_IN_EMPTY] THEN CONV_TAC "TAUT" TAUT;
    ALL_TAC] THEN
 ...
 (`!p:real^N->bool.
  ...
  ANTS_TAC THENL
   [MATCH_MP_TAC SURJECTIVE_IMAGE_EQ THEN
    ...
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC CARD_IMAGE_INJ THEN CONJ_TAC THENL
