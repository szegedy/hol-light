...
let DESARGUES_DIRECT = prove
 (`~COLLINEAR {A',B,S} /\
  ...
  (CONV_TAC "(ONCE_DEPTH_CONV BRACKET_SWAP_CONV)") (ONCE_DEPTH_CONV BRACKET_SWAP_CONV) THEN
  ...
  CONJ_TAC THENL [POP_ASSUM MP_TAC THEN (CONV_TAC "REAL_RING") REAL_RING; ALL_TAC] THEN
