...
let ALPHAVARS_REFL = prove
 (`!env t. ALL (\(s,t). s = t) env ==> ALPHAVARS env (t,t)`,
  ...
  CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN MESON_TAC[PAIR_EQ]);;
 ...
 (`!t env. ALL (\(s,t). s = t) env ==> RACONV env (t,t)`,
  ...
  REWRITE_TAC[RACONV] THEN REPEAT STRIP_TAC THENL
   [ASM_SIMP_TAC[ALPHAVARS_REFL];
    ...
    CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN ASM_REWRITE_TAC[]]);;
...
let VFREE_IN_FINITE_ALT = prove
 (`!t ty. FINITE {x | VFREE_IN (Var x ty) t}`,
  ...
  EXISTS_TAC `Var x ty` THEN CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
  ASM_REWRITE_TAC[]);;
