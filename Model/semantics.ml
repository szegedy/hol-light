 ...
 (`!tm sigma tau ilist.
  ...
  REWRITE_TAC[VALMOD] THENL
   [ALL_TAC;
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
 ...
 (`!n tm env tyin.
  ...
  REWRITE_TAC[DEST_VAR] THEN REWRITE_TAC[CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_REWRITE_TAC[] THEN CONJ_TAC THEN
    ...
    MAP_EVERY X_GEN_TAC [`k:string`; `kty:type`] THEN
    REWRITE_TAC[VALMOD] THEN CONV_TAC "(ONCE_DEPTH_CONV GEN_BETA_CONV)" (ONCE_DEPTH_CONV GEN_BETA_CONV) THEN
...
let HOL_IS_CONSISTENT = prove
 (`?p. p has_type Bool /\ ~([] |- p)`,
  ...
  CONV_TAC "SELECT_CONV" SELECT_CONV THEN MATCH_MP_TAC TYPESET_INHABITED THEN
