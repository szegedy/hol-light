 ...
 (`!x n k. abs(x) <= inv(&2 pow k) /\ ~(k = 0)
  ...
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [ASM_REWRITE_TAC[sum; REAL_SUB_RZERO; MULT_CLAUSES; SUB_0] THEN
    ...
    REWRITE_TAC[real_pow; REAL_POW_1] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
    CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
    ...
    CONV_TAC "(RAND_CONV REAL_RAT_REDUCE_CONV)" (RAND_CONV REAL_RAT_REDUCE_CONV) THEN REWRITE_TAC[REAL_POW_1] THEN
...
let pi_approx_rule,PI_APPROX_RULE =
  ...
  let pth = prove
   (`(q1 = p + 5) /\
    ...
    CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV) in
