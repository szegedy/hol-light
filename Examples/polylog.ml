...
let MAGIC_DERIVATIVE = prove
 (`!x. abs(x) < &1
  ...
  CONV_TAC "(TOP_DEPTH_CONV num_CONV)" (TOP_DEPTH_CONV num_CONV) THEN
  REWRITE_TAC[real_pow] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN
  REAL_ARITH_TAC);;
...
let POLYLOG_DERIVATIVE = prove
 (`!a b x. ~(a = 0) /\ ~(b = 0) /\ abs(x) < &1
  ...
  CONV_TAC "(LAND_CONV(ONCE_DEPTH_CONV(ALPHA_CONV `n:num`)))" (LAND_CONV(ONCE_DEPTH_CONV(ALPHA_CONV `n:num`))) THEN
 ...
 (`(\n. inv(&16 pow n) * (&4 / &(8 * n + 1) -
  ...
  ASSUME_TAC THENL
   [REPEAT STRIP_TAC THEN
    ...
    CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_REWRITE_TAC[real_pow] THEN
    DISCH_THEN(MP_TAC o SPEC `&4 * sqrt(&2)` o MATCH_MP DIFF_CMUL) THEN
    MP_TAC(SPECL [`8`; `4`; `x:real`] POLYLOG_DERIVATIVE) THEN
    CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV THEN ASM_REWRITE_TAC[] THEN
    ...
    ASM_REWRITE_TAC[] THEN CONV_TAC "NUM_REDUCE_CONV" NUM_REDUCE_CONV; ALL_TAC] THEN
  MP_TAC(SPEC
   `\x. suminf (\n. &4 * sqrt (&2) * x pow (8 * n + 1) / &(8 * n + 1) -
                    &8 * x pow (8 * n + 4) / &(8 * n + 4) -
                    &4 * sqrt (&2) * x pow (8 * n + 5) / &(8 * n + 5) -
   ...
   [SIMP_TAC[SQRT_POS_LT; REAL_LT_INV_EQ; REAL_OF_NUM_LT; ARITH] THEN
    ...
    SUBGOAL_THEN `abs(x) < &1` MP_TAC THENL
     [MATCH_MP_TAC(REAL_ARITH
      ...
      MATCH_MP_TAC SQRT_MONO_LE THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV;
      ALL_TAC] THEN
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[LN_1; REAL_ADD_LID] THEN
  REWRITE_TAC[GSYM REAL_ADD_LDISTRIB] THEN
  ONCE_REWRITE_TAC[ADD_SYM] THEN
  ...
  MP_TAC THENL
   [MATCH_MP_TAC SER_COMPAR THEN
    ...
    MATCH_MP_TAC SER_CMUL THEN
    MATCH_MP_TAC GP THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV; ALL_TAC] THEN
  ...
  REWRITE_TAC[ATN_TAN] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
