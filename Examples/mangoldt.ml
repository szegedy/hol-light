...
let LOG_2_BOUNDS = prove
 (`&1 / &2 <= log(&2) /\ log(&2) <= &1`,
  ...
  CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC LOG_MONO_LE THEN
...
let LOG_FACT_BOUNDS = prove
 (`!n. ~(n = 0)
   ...
   [MP_TAC(ISPECL[`\z. clog(z)`; `\z. z * clog z - z`; `1`; `n:num`]
    ...
    REWRITE_TAC[] THEN ANTS_TAC THENL
       ...
       [REWRITE_TAC[IN_SEGMENT_CX_GEN] THEN REPEAT STRIP_TAC THENL
         [COMPLEX_DIFF_TAC THEN CONJ_TAC THEN UNDISCH_TAC `&1 <= Re x` THENL
          ...
          ASM_CASES_TAC `x = Cx(&0)` THEN ASM_REWRITE_TAC[RE_CX] THENL
           [REAL_ARITH_TAC;
            UNDISCH_TAC `~(x = Cx(&0))` THEN CONV_TAC "COMPLEX_FIELD" COMPLEX_FIELD];
     ...
     FIRST_ASSUM(DISJ_CASES_TAC o MATCH_MP (ARITH_RULE
      `~(n = 0) ==> n = 1 \/ 2 <= n`))
     THENL
      [ASM_REWRITE_TAC[] THEN CONV_TAC "(ONCE_DEPTH_CONV NUMSEG_CONV)" (ONCE_DEPTH_CONV NUMSEG_CONV) THEN
  ...
  CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN REWRITE_TAC[REAL_SUB_RNEG] THENL
 ...
 (`!n. ~(n = 0) ==> log(&n) = sum {d | d divides n} (\d. mangoldt(d))`,
  ...
  MATCH_MP_TAC INDUCT_COPRIME THEN REPEAT STRIP_TAC THENL
   [ASM_SIMP_TAC[LOG_MUL; GSYM REAL_OF_NUM_MUL; REAL_OF_NUM_LT;
    ...
    CONJ_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THENL
 ...
 (`!n. log(&(FACT n)) = sum(1..n) (\d. mangoldt(d) * floor(&n / &d))`,
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_EQ_NUMSEG THEN X_GEN_TAC `m:num` THEN
    STRIP_TAC THEN REWRITE_TAC[] THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
 ...
 (`!n. ~(n = 0) ==> abs(sum(1..n) (\d. mangoldt(d) / &d) - log(&n)) <= &21`,
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC(REAL_ARITH `&1 <= x ==> abs(x - &1) <= x`) THEN
    ...
    ALL_TAC] THEN
  REWRITE_TAC[GSYM SUM_SUB_NUMSEG] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  ASM_SIMP_TAC[LOG_LE_REFL] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
...
let MERTENS_MANGOLDT_VERSUS_LOG = prove
 (`!n s. s SUBSET (1..n)
   ...
   [SUBGOAL_THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC SUM_SUPERSET THEN
      ...
      ALL_TAC] THEN
    CONV_TAC "SYM_CONV" SYM_CONV THEN MATCH_MP_TAC SUM_UNION_EQ THEN CONJ_TAC THENL
  ...
  ASM_SIMP_TAC[SUM_CLAUSES] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN
  ...
  ASM_REWRITE_TAC[] THEN CONV_TAC "REAL_RAT_REDUCE_CONV" REAL_RAT_REDUCE_CONV THEN ANTS_TAC THENL
   [CONJ_TAC THENL
