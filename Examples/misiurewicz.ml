 ...
 (`!z. abs(Im z) <= pi / &3 ==> Re(cexp z) >= Re z + (&1 - log(&2))`,
  ...
  TRANS_TAC REAL_LE_TRANS `exp(Re z) / &2` THEN CONJ_TAC THENL
   [ALL_TAC;
    ...
    REWRITE_TAC[COS_PI3] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV] THEN
...
let LEMMA_2b = prove
 (`!z. ~(real z) ==> ?n. abs(Im(ITER n cexp z)) > pi / &3`,
  ...
  MP_TAC(SPECL [`&2`; `pi / &3 / abs(Im (ITER N cexp z))`]
        REAL_ARCH_POW) THEN
  ASM_SIMP_TAC[REAL_LT_LDIV_EQ] THEN (CONV_TAC "REAL_RAT_REDUCE_CONV") REAL_RAT_REDUCE_CONV THEN
