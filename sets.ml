 ...
 (`!P (R:A->B->C->bool).
  ...
  REPEAT STRIP_TAC THEN BETA_TAC THEN EQ_TAC THENL
   [STRIP_TAC THEN MATCH_MP_TAC SELECT_UNIQUE THEN ASM_MESON_TAC[];
    DISCH_THEN(SUBST1_TAC o SYM) THEN CONV_TAC "SELECT_CONV" SELECT_CONV THEN
    ASM_MESON_TAC[]]);;
...
let FINITE_RECURSION = prove
 (`!(f:A->B->B) b.
  ...
  CONV_TAC "SELECT_CONV" SELECT_CONV THEN MATCH_MP_TAC SET_RECURSION_LEMMA THEN
  ASM_REWRITE_TAC[]);;
 ...
 (`!d n t:B->bool m s:A->bool.
  ...
  REWRITE_TAC[HAS_SIZE_CLAUSES] THENL
   [REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[NOT_IN_EMPTY; EXP] THEN
    CONV_TAC "HAS_SIZE_CONV" HAS_SIZE_CONV THEN EXISTS_TAC `(\x. d):A->B` THEN
...
let HAS_SIZE_BOOL = prove
 (`(:bool) HAS_SIZE 2`,
  SUBGOAL_THEN `(:bool) = {F,T}` SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_UNIV; IN_INSERT] THEN CONV_TAC "TAUT" TAUT;
 ...
 (`!(s:A->bool) n. s HAS_SIZE n ==> {t | t SUBSET s} HAS_SIZE (2 EXP n)`,
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; IN_ELIM_THM; IN_UNIV; SUBSET; IN; CONTRAPOS_THM];
    ...
    CONV_TAC "HAS_SIZE_CONV" HAS_SIZE_CONV THEN MAP_EVERY EXISTS_TAC [`T`; `F`] THEN
 ...
 (`(!a. ~FINITE {x:real | a < x}) /\
  ...
  ASSUME_TAC THENL
   [REPEAT GEN_TAC THEN REWRITE_TAC[GSYM REAL_NOT_LT] THEN
    ...
    MATCH_MP_TAC EQ_IMP THEN AP_TERM_TAC THEN CONV_TAC "SYM_CONV" SYM_CONV THEN
...
let HAS_SIZE_INDEX = prove
 (`!s n. s HAS_SIZE n
  ...
  ASM_REWRITE_TAC[IN_DELETE] THEN
  CONV_TAC "(ONCE_DEPTH_CONV COND_ELIM_CONV)" (ONCE_DEPTH_CONV COND_ELIM_CONV) THEN
...
let LIST_OF_SET_PROPERTIES = prove
 (`!s:A->bool. FINITE(s)
  ...
  CONV_TAC "(BINDER_CONV(RAND_CONV SELECT_CONV))" (BINDER_CONV(RAND_CONV SELECT_CONV)) THEN
...
let SUP = prove
 (`!s. ~(s = {}) /\ (?b. !x. x IN s ==> x <= b)
  ...
  REWRITE_TAC[sup] THEN CONV_TAC "(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
...
let INF = prove
 (`!s. ~(s = {}) /\ (?b. !x. x IN s ==> b <= x)
  ...
  CONV_TAC "(ONCE_DEPTH_CONV SELECT_CONV)" (ONCE_DEPTH_CONV SELECT_CONV) THEN
